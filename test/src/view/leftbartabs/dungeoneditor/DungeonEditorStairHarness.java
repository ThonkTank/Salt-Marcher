...
final class DungeonEditorStairHarness {

    private static final String OWNER = "DungeonEditorStairHarness";

    private DungeonEditorStairHarness() {
    }

    static void run(List<String> results) throws Exception {
        route(results, () -> verifyStraightStairCreateThroughMapView(results));
        route(results, () -> verifyStairAnchorMoveThroughMapView(results));
        route(results, () -> verifyStraightStairGeometryEditThroughStateView(results));
        route(results, () -> verifyInvalidStairRoomInteriorRecomputeThroughStateView(results));
        route(results, () -> verifyStairDeleteThroughMapView(results));
        route(results, () -> verifyCrossLevelCorridorCreatesStairThroughMapView(results));
    }
