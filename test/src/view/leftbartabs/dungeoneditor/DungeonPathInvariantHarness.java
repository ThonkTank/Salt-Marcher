...
final class DungeonPathInvariantHarness {
    ...
    private static final String OWNER = "PathInvariantHarness";

    private DungeonPathInvariantHarness() {
    }
    ...
    static void run(List<String> results) {
        ...
        DungeonEditorBehaviorHarnessSupport.recordModelInvariant(
                ...
                OWNER,
                "DGI-PATH-001",
                "Shared path primitives normalize cell sequences and stay passive");
        assertCorridorPathOwner();
        DungeonEditorBehaviorHarnessSupport.recordModelInvariant(
                results,
                OWNER,
                "DGI-PATH-002",
                "Corridor route owner validates routes, detects blocked paths, and plans interior waypoints");
        assertStairPathOwner();
        DungeonEditorBehaviorHarnessSupport.recordModelInvariant(
                results,
                OWNER,
                "DGI-PATH-003",
                "Stair path owner keeps generated path cells, exits, readability, and room-interior predicates");
