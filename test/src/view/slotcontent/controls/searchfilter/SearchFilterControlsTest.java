...
public final class SearchFilterControlsTest {
    ...
    void hideWindows() throws Exception {
        runOnFxThread(SearchFilterControlsTest::hideOpenWindows);
    }
    ...
    private static void runOnFxThread(ThrowingRunnable action) throws Exception {
        ...
        if (!latch.await(AWAIT_SECONDS, TimeUnit.SECONDS)) {
            throw new IllegalStateException("Timed out waiting for JavaFX SearchFilter test.");
        }
        if (failure[0] != null) {
            throw new IllegalStateException("SearchFilter controls test failed.", failure[0]);
        }
    }
