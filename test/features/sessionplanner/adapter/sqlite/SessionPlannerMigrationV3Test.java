...
final class SessionPlannerMigrationV3Test {
    ...
    private static void createVersionTwoFixture(Path path) throws Exception {
        ...
        try (Connection connection = DriverManager.getConnection("jdbc:sqlite:" + path);
            ...
            statement.execute(
                    "CREATE TABLE session_planner_sessions (session_id INTEGER PRIMARY KEY,"
                        + " display_name TEXT NOT NULL, encounter_days TEXT NOT NULL,"
                        + " selected_encounter_id INTEGER NOT NULL, status_text TEXT NOT NULL,"
                        + " next_encounter_id INTEGER NOT NULL, next_loot_id INTEGER NOT NULL,"
                        + " updated_at TEXT NOT NULL)");
            statement.execute(
                    "CREATE TABLE session_planner_current_session (singleton_id INTEGER PRIMARY"
                        + " KEY, session_id INTEGER REFERENCES"
                        + " session_planner_sessions(session_id))");
            statement.execute(
                    "CREATE TABLE session_planner_participants (session_id INTEGER NOT NULL"
                        + " REFERENCES session_planner_sessions(session_id) ON DELETE CASCADE,"
                        + " character_id INTEGER NOT NULL, sort_order INTEGER NOT NULL, PRIMARY"
                        + " KEY(session_id, character_id))");
            statement.execute(
                    "CREATE TABLE session_planner_encounters (session_id INTEGER NOT NULL"
                        + " REFERENCES session_planner_sessions(session_id) ON DELETE CASCADE,"
                        + " encounter_id INTEGER NOT NULL, encounter_plan_id INTEGER NOT NULL,"
                        + " budget_percentage TEXT NOT NULL, scene_title TEXT NOT NULL, scene_notes"
                        + " TEXT NOT NULL, location_id INTEGER NOT NULL, sort_order INTEGER NOT"
                        + " NULL, PRIMARY KEY(session_id, encounter_id))");
            statement.execute(
                    "CREATE TABLE session_planner_rests (session_id INTEGER NOT NULL REFERENCES"
                        + " session_planner_sessions(session_id) ON DELETE CASCADE,"
                        + " left_encounter_id INTEGER NOT NULL, right_encounter_id INTEGER NOT"
                        + " NULL, rest_kind TEXT NOT NULL, sort_order INTEGER NOT NULL, PRIMARY"
                        + " KEY(session_id, left_encounter_id, right_encounter_id))");
            statement.execute(
                    "CREATE TABLE session_planner_loot_placeholders (session_id INTEGER NOT NULL"
                        + " REFERENCES session_planner_sessions(session_id) ON DELETE CASCADE,"
                        + " loot_id INTEGER NOT NULL, encounter_id INTEGER NOT NULL, label TEXT NOT"
                        + " NULL, sort_order INTEGER NOT NULL, PRIMARY KEY(session_id, loot_id))");
            statement.execute(
                    "CREATE TABLE session_planner_generated_rewards (session_id INTEGER NOT NULL"
                        + " REFERENCES session_planner_sessions(session_id) ON DELETE CASCADE,"
                        + " scene_id INTEGER NOT NULL, generation_id TEXT NOT NULL, treasure_id"
                        + " INTEGER NOT NULL, last_known_label TEXT NOT NULL, sort_order INTEGER"
                        + " NOT NULL, PRIMARY KEY(session_id, generation_id, treasure_id), FOREIGN"
                        + " KEY(session_id, scene_id) REFERENCES"
                        + " session_planner_encounters(session_id, encounter_id))");
