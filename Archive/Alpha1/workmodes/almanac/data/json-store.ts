...
const logger = configurableLogger.forModule('almanac-json-store');

// VaultLike interface for testing - minimal subset of Vault API
...
export class JsonStore<T> {
  ...
  constructor(appOrVault: App | VaultLike, private readonly config: JsonStoreConfig<T>) {
    this.normalizedPath = normalizePath(config.path);
    // Check if it's an App (has vault property) or directly a VaultLike
  ...
  private async ensurePayload(): Promise<VersionedPayload<T>> {
    ...
    try {
      ...
      logger.warn(`Failed to parse ${this.normalizedPath}, resetting file`, error);
      const fallback: VersionedPayload<T> = { version: this.config.currentVersion, data: this.config.initialData() };
      await this.writePayload(fallback);
      return fallback;
    }
