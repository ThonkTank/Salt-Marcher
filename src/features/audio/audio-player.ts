...
import type { PlaylistData, AudioTrack } from "../../workmodes/library/playlists/types";
import type { AudioPlayer, PlaybackState, PlaybackStatus, CurrentTrack } from "./types";
...
export function createAudioPlayer(): AudioPlayer {
	...
	function handleTrackEnded(): void {
		...
		advanceToNextTrack().catch((error) => {
			logger.error("[AudioPlayer] Failed to advance to next track", { error });
		});
	}
	...
	function handleAudioError(event: Event): void {
		...
		logger.error("[AudioPlayer] Audio error", {
			src: audio.src,
			error: audio.error?.message,
		});
		// Try next track
		advanceToNextTrack().catch((error) => {
			logger.error("[AudioPlayer] Failed to recover from audio error", { error });
		});
	}
	...
	function notifyStatusChange(): void {
		...
		statusSubscribers.forEach((callback) => {
			try {
				...
				logger.error("[AudioPlayer] Status callback error", { error });
			}
		});
	}
	...
	async function skipToTrackInternal(index: number): Promise<void> {
		if (!playlist || index < 0 || index >= playlist.tracks.length) {
			logger.error("[AudioPlayer] Invalid track index", { index });
			return;
		}
	...
	const api: AudioPlayer = {
		loadPlaylist(newPlaylist: PlaylistData): void {
			logger.info("[AudioPlayer] Loading playlist", { name: newPlaylist.name });
		...
		async play(): Promise<void> {
			if (!playlist || playlist.tracks.length === 0) {
				logger.warn("[AudioPlayer] Cannot play: no playlist loaded");
				return;
			}
			...
			const track = playlist.tracks[currentTrackIndex];
			if (!track) {
				logger.error("[AudioPlayer] Invalid track index", { currentTrackIndex });
				return;
			}
		...
		toggleShuffle(): void {
			...
			logger.info("[AudioPlayer] Shuffle toggled", { shuffle });
			notifyStatusChange();
		},

		toggleLoop(): void {
			loop = !loop;
			logger.info("[AudioPlayer] Loop toggled", { loop });
			notifyStatusChange();
		},

		setCrossfadeDuration(seconds: number): void {
			crossfadeDuration = Math.max(0, Math.min(10, seconds));
			logger.info("[AudioPlayer] Crossfade duration set", { crossfadeDuration });
			notifyStatusChange();
		},
		...
		dispose(): void {
			logger.info("[AudioPlayer] Disposing");
