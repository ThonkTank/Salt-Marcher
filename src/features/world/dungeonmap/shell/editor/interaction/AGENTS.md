...
- `ConnectionsTool` owns door edits, surface-driven corridor authoring, corridor graph delete gestures, and the shared stair draft/edit UX consumed by both connection editing and stair selection. Boundary eligibility for door-like placement is shared with `TransitionTool` through `ConnectionSurfaceSupport`; authoritative transition/local-connection construction stays out of the tools.
