...
- `SelectionTool` owns semantic selection, cluster drag, whole-stair drag, door drag, and corridor-node drag. Physical door hits resolve through `DungeonSelectionRef.DoorRef`; door drag starts from that canonical door ref and still drops onto `RoomBoundaryRef`. Do not restore legacy connection-based door drag subjects.
...
- `CorridorTool` owns corridor authoring and corridor graph deletion. Corridors start from existing exterior room doors via canonical `DoorRef`, may attach to a free corridor wall on the second click, and right click deletes hovered corridor-owned segments, nodes, or endpoint doors. Pending corridor starts stay tool-local and are cancelled with right click.
...
- `TransitionTool` owns transition create/delete gestures and its local form state. Door placement accepts only existing canonical `DungeonSelectionRef.DoorRef` subjects; stair placement still starts from `FloorCellRef`. Final `DungeonConnection` construction/validation happens through the shared transition connection builder, not in the tool. Selected targets stay as `DungeonTransitionDestination`, not split enum/id fields.
