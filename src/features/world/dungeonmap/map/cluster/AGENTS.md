...
- `model/RoomCluster` — top-level cluster aggregate that extends `Structure` with cluster identity, center, and room metadata membership.
...
- Put cluster identity, center, structure-backed boundary edits, and cluster-local move semantics on `RoomCluster`.
...
- Do not add second cluster mutation seams beside `RoomCluster` and `DungeonClusterApplicationService`.
