package features.world.dungeonmap.map.cluster.application;

import database.DatabaseManager;
import features.world.dungeonmap.application.support.DungeonTransactionRunner;
import features.world.dungeonmap.map.cluster.model.ClusterStructureEditor;
...
import features.world.dungeonmap.map.structure.model.surface.StructureSurface;
...
public final class DungeonClusterApplicationService {
    ...
    public void paintCells(Connection conn, long mapId, int levelZ, GridArea cells) throws SQLException {
        ...
        List<RoomCluster> overlappingClusters = overlappingClustersAtLevel(layout, requestedCells, levelZ).stream()
        ...
        RoomCluster mergedCluster = ClusterStructureEditor.applyPaint(
    ...
    public void deleteCells(Connection conn, long mapId, int levelZ, GridArea cells) throws SQLException {
        ...
        for (Long clusterId : affectedClusterIds) {
            ...
            List<RoomCluster> finalClusters = ClusterStructureEditor.assignGeneratedClusterRoomNames(
    ...
    public void move(long mapId, long clusterId, GridTranslation translation) throws SQLException {
        ...
        try (Connection conn = DatabaseManager.getConnection()) {
            DungeonTransactionRunner.inTransaction(conn, () -> {
                DungeonLayout layout = requireLayout(conn, mapId);
                DungeonLayout movedLayout = layout.withMovedCluster(clusterId, resolvedTranslation);
    ...
    public void createWallPath(
        ...
        RoomCluster updatedCluster = cluster.createWallPath(levelZ, segments);
        if (updatedCluster == null) {
            return;
        }
    ...
    public void deleteWallPath(
        ...
        RoomCluster updatedCluster = cluster.deleteWallPath(levelZ, segments);
        if (updatedCluster == null) {
            return;
        }
    ...
    private void saveMovedCluster(Connection conn, RoomCluster cluster) throws SQLException {
    ...
    private DungeonLayout loadRoomRewriteLayout(Connection conn, DungeonLayout originalLayout, long mapId) throws SQLException {
        ...
        List<RoomCluster> clusters = clusterRepository.loadClusters(conn, mapId, rooms);
    ...
    private static List<RoomCluster> normalizedClusters(List<RoomCluster> clusters) {
        ...
        List<RoomCluster> result = new java.util.ArrayList<>();
