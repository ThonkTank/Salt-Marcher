...
- `StructureSurface` — level-local surface owner for anchor, surface cells, floor cells, and surface queries.
...
- Put level-local surface behavior on `StructureSurface`.
- Put level-local wall, door, and boundary-edge behavior on `StructureBoundary`.
...
- Do not bypass `StructureSurface` by rebuilding anchor, surface, floor, or clipping logic ad hoc.
