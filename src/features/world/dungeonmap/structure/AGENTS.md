...
- `Structure` — shared physical structure aggregate — composes the local `surface`, `boundary`, and `room` sub-owners for each level. `roomTopology()` is a derived companion over the physical structure plus room metadata, not an additional persisted structure state.
