...
- `Corridor` keeps its node/segment graph and room-bound endpoint cells as authored truth. Routed path traces, occupied cells, boundary/opening geometry, and related topology queries are derived only through `StructureObject`, including room-bound openings and persisted free corridor boundary doors. Junction nodes are explicit authored state: door-to-corridor attachments and tile-drags may create them, but routing itself must not invent extra nodes.
...
- transitions: `dungeon_transitions` with a nullable placed-connection discriminator plus either door-carrier fields or stair geometry/reopen rows with generic shape-spec columns, alongside the destination discriminator
