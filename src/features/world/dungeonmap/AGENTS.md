...
- `Floor` extends `TileShape`, `Wall` and `Door` extend `EdgeShape`, and `Stair` owns a `TilePath` plus derived occupied cells. Do not reintroduce parallel wrapper hierarchies that carry the same tiles, edges, or ordered path outside those seams.
...
- `objects/` owns thin domain objects over geometry such as `Floor`, `Wall`, `Door`, `StructureObject`, and `StructureDescriptor`. `Floor`/`Stair` stay tile-shaped, `Wall`/`Door` stay segment-shaped, and shared boundary queries still operate on projected `GridSegment2x`.
...
- `Stair` is a first-class structure with stable identity, authored stop levels, and direct `TilePath` ownership over its ordered occupied path. Occupied cells and exits are derived views from that path plus stop levels, not persisted second truths.
