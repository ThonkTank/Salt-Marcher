...
- `Traversal` is the canonical owner of connection edit truth. Membership changes, waypoint edits, door-binding edits, and binding re-anchoring belong on `Traversal`/`CorridorBindings`, not on `Corridor`. Corridor and stair objects are materialized read models
