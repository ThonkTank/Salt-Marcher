...
- `Floor` extends `TileShape`, `Wall` and `Door` extend `EdgeShape`, and `Stair` extends `TilePath`. Those typed objects add semantics, while topology math lives on the shared shape bases. Do not reintroduce parallel wrapper hierarchies that carry the same tiles, edges, or ordered path outside those seams.
...
- `StructureObject` is the topology orchestrator over shape-backed `Floor`, `Wall`, `Door`, and optional `Stair` projections. Corridors and clusters keep one canonical `StructureObject`; room-local structures are derived on demand from cluster truth plus room metadata.
...
- `objects/` owns thin domain objects over geometry such as `Floor`, `Wall`, `Door`, `StructureObject`, and `StructureDescriptor`. `Floor` stays tile-shaped, `Stair` stays path-shaped, `Wall`/`Door` stay segment-shaped, and shared boundary queries still operate on projected `GridSegment2x`.
...
- `Stair` is a first-class structure with stable identity, authored stop levels, and direct `TilePath` inheritance over its ordered occupied path. Occupied cells and exits are derived views from that path plus stop levels, not persisted second truths.
