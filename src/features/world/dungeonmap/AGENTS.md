...
- **TRAVERSAL_CREATE / TRAVERSAL_DELETE** → `TraversalTool` — two-click flow via `EditorDraft.TraversalDraft`; targets may be room, corridor segment, or stair segment. Create resolves through `DungeonTraversalApplicationService`, delete removes the parent traversal via the clicked corridor/stair segment
