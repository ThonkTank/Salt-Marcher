...
- `StructureDescriptor.LevelDescriptor` authors cluster/corridor surface truth as `anchorCell`, `surfaceShape`, `boundaryShape`, `openingShape`, and an optional floor `TileShape` subset. Rooms persist only per-level anchors and derive their own projected `StructureObject` from the enclosing cluster topology. Compatibility getters such as `boundaryEdges()` and `floorCells()` are projections from those shapes, not a second canonical geometry owner.
...
- `objects/` owns thin domain objects over geometry such as `Floor`, `Wall`, `Door`, passive `Stair`, `StructureObject`, and `StructureDescriptor`. `Floor` stays tile-shaped, `Stair` stays path-shaped, `Wall`/`Door` stay segment-shaped, and shared boundary queries still operate on projected `GridSegment2x`.
