...
- **`Corridor`** — first-class horizontal structure: corridorId, mapId, canonical stored roomIds, one `levelZ`, ordered corridor points, and corridor-local endpoint bindings. `Corridor` derives its own `CorridorPath`, occupied cells/floors, and corridor-local `Connection`s. Endpoint bindings describe corridor-local connections but do not answer corridor room membership. It may participate in traversal-organized segment refs, but it is not traversal-owned read-model state
