...
- When model callers work with one `Door` or `Wall`, use that object's explicit API for clipping, segment access, touching-cell reads, or rewrite behavior instead of composing generic `EdgeShape` edits locally.
...
- Do not call inherited generic `EdgeShape` methods on `Door` or `Wall` from model code; if a needed read is missing, add it to `BoundaryObject`, `Door`, or `Wall` instead of widening the caller.
