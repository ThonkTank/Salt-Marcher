...
- `StructureObject` is the topology orchestrator over shape-backed `Floor`, `Wall`, `Door`, optional passive `Stair`, and derived corridor path-trace projections. Boundary classification, reachability, clipped-surface derivation, connected-component splits, and routed corridor trace math belong there. It may expose raw cell/edge projections only as compatibility views, and it is the only allowed bridge from `objects/` into `structures/`.
...
- `RoomCluster` owns cluster metadata plus room/door workflow semantics over `StructureObject`-owned topology queries. Adjacency, paint/delete/boundary mutation semantics, cluster moves, and derived local connections stay there.
...
- Corridor room-bound endpoints keep absolute `CellCoord` room cells in memory. The graph resolves into the same `StructureObject.LevelStructure` and `StructureObject` surface model used by cluster-derived rooms, with routed path traces and opening/boundary projections owned by `StructureObject`, including room-bound openings and persisted free boundary doors.
