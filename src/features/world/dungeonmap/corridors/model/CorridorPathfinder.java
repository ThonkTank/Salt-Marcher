...
final class CorridorPathfinder {
    ...
    private static List<Point2i> shortestPath(
        ...
        int shortestDistance = shortestDistance(start, goal, grid);
        if (shortestDistance < 0) {
            return List.of();
        }
        ...
        while (!open.isEmpty()) {
            ...
            for (int directionIndex = 0; directionIndex < CorridorRouteGeometry.CARDINAL_NEIGHBORS.size(); directionIndex++) {
                Point2i direction = CorridorRouteGeometry.CARDINAL_NEIGHBORS.get(directionIndex);
