...
public final class DungeonView implements AppView {
    ...
    private void selectCorridor(DungeonCorridor corridor) {
        ...
        CorridorComponent component = currentState == null || currentState.layout() == null
                ? null
                : DungeonCorridorGeometry.corridorTopology(currentState.layout()).componentForCorridor(corridor.corridorId());
        if (component == null) {
            return;
        }
    ...
    private DungeonCorridorSummary corridorSummary(DungeonCorridor corridor) {
        CorridorComponent component = DungeonCorridorGeometry.corridorTopology(currentState.layout()).componentForCorridor(corridor.corridorId());
    ...
    private String activeLocationLabel(DungeonLayout layout, DungeonRuntimeLocation location) {
        ...
        if (location instanceof DungeonRuntimeLocation.CorridorComponent corridorComponent) {
            CorridorComponent component = DungeonCorridorGeometry.corridorTopology(layout).componentById(corridorComponent.componentId());
            if (component == null) {
                return null;
            }
