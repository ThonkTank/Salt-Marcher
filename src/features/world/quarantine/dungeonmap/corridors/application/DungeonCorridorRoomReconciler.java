...
public final class DungeonCorridorRoomReconciler implements CorridorRoomReconciler {
    ...
    public void reconcileRoomCorridors(Connection conn, DungeonLayout layout, long mapId, long originalRoomId, List<DungeonRoom> fragments) throws SQLException {
        ...
        for (DungeonCorridor corridor : layout.corridors()) {
            ...
            DungeonRoom targetFragment = chooseBestCorridorFragment(layout, corridor, originalRoomId, fragments);
            List<Long> replacedRoomIds = corridor.roomIds().stream()
    ...
    private static DungeonRoom chooseBestCorridorFragment(DungeonLayout layout, DungeonCorridor corridor, long originalRoomId, List<DungeonRoom> fragments) {
        ...
        for (DungeonRoom fragment : fragments) {
            int nearestRoomDistance = corridor.roomIds().stream()
                    ...
                    .mapToInt(room -> fragment.distanceTo(room))
                    .min()
