...
public final class DungeonStorageSupport {
    ...
    public static void createSchema(Statement stmt) throws SQLException {
        ...
        stmt.execute("CREATE TABLE IF NOT EXISTS dungeon_corridors ("
                ...
                + "dungeon_map_id   INTEGER NOT NULL REFERENCES dungeon_maps(dungeon_map_id) ON DELETE CASCADE,"
                + "structure_object_id INTEGER NOT NULL UNIQUE REFERENCES dungeon_structure_objects(structure_object_id),"
                + "root_door_id     INTEGER REFERENCES dungeon_structure_doors(door_id),"
                + "root_point_x2    INTEGER,"
                + "root_point_y2    INTEGER,"
                + "CHECK((root_door_id IS NOT NULL AND root_point_x2 IS NULL AND root_point_y2 IS NULL)"
                + " OR (root_door_id IS NULL AND root_point_x2 IS NOT NULL AND root_point_y2 IS NOT NULL))"
                + ")");
        ...
        stmt.execute("CREATE TABLE IF NOT EXISTS dungeon_corridor_members ("
                ...
                + "host_waypoint_id      INTEGER,"
                + "CHECK((door_id IS NOT NULL AND terminal_x2 IS NULL AND terminal_y2 IS NULL)"
                + " OR (door_id IS NULL AND terminal_x2 IS NOT NULL AND terminal_y2 IS NOT NULL)),"
                + "CHECK((host_member_id IS NULL AND host_waypoint_id IS NULL)"
                + " OR (host_member_id IS NOT NULL AND host_waypoint_id IS NOT NULL)),"
                + "UNIQUE(corridor_id, member_id)"
                + ")");
        stmt.execute("CREATE TABLE IF NOT EXISTS dungeon_corridor_waypoints ("
                ...
                + "UNIQUE(corridor_id, waypoint_id),"
                + "UNIQUE(corridor_id, member_id, waypoint_order),"
                + "FOREIGN KEY(corridor_id, member_id) REFERENCES dungeon_corridor_members(corridor_id, member_id) ON DELETE CASCADE"
                + ")");
