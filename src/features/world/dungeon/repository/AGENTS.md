...
`repository` owns the remaining legacy owner-local dungeon persistence seams that still live directly beneath `dungeon/`. Map rehydration now lives in the sibling `dungoenmap/repository` slice.

## Canonical Types and APIs

- `DungeonRoomRepository` — persists room metadata and room-to-cluster membership.
- `dungoenmap/cluster/DungeonClusterRepository` — persists top-level cluster rows and cluster-owned structure references.
...
- Put loaded-map rehydration in `dungoenmap/repository/`, not back in this legacy root.
- Route shared physical structure persistence through `dungoenmap/structure/repository` and the canonical `Structure` snapshot.
