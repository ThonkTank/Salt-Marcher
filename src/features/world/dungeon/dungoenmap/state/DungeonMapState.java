package features.world.dungeon.dungoenmap.state;
...
public final class DungeonMapState {
    ...
    public void showLoaded(List<DungeonMapCatalogEntry> maps, DungeonMap activeMap, String errorMessage) {
        Long previousMapId = activeMapId;
        this.maps = maps == null ? List.of() : List.copyOf(maps);
        this.activeMap = activeMap == null ? DungeonMap.empty() : activeMap;
    ...
    private static int resolvedProjectionLevel(DungeonMap activeMap, int preferred) {
        if (activeMap == null) {
            return 0;
        }
    ...
    private static int defaultProjectionLevel(DungeonMap activeMap) {
