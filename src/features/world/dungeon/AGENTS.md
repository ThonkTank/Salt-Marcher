...
- `checkOwnerApiBoundaryConvention` — build-time touched-file architecture gate — touched files are validated against the role of their direct home directory, every owner must expose a single root `*Object` seam, owner internals may use only the flat `input`, `task`, `repository`, and `state` layers plus optional direct-child `*Bucket` organization directories, and request-boundary types must use direct `<Request>Input` / `<Request>Task` naming that matches a real public request on the owning `*Object`.
...
- Put owner-local transformation pipelines under `owner/task`; task files stay static, model one `input -> input` pipeline, and do not own persistence or direct state mutation.
...
- Route authoritative reloads through the owning `*Object` orchestration plus the owner's `repository` and `state` layers.
