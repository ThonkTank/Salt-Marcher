...
- `model/CorridorInput`, `model/CorridorInputNode`, `model/CorridorSegment` — canonical persisted corridor-authored input — describe the ordered node-and-segment network that drives routing and final structure generation.
...
- `model/CorridorRouting` — canonical corridor routing helper — projects authored node anchors plus authored segments into routed structure and trace output and rehydrates transient traces back from persisted structure plus the authored input network.
