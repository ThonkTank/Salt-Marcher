...
This file covers `src/features/world/dungeon/dungeonmap/structure/model/room/`.
...
- Do not mirror room lookup, room containment, or derived room-structure reads back onto `Cluster` or `DungeonMap`; callers must resolve the owning structure and continue on `roomTopology()`.
