...
- `model/CorridorResolutionRequest`, `model/CorridorRehydrationRequest` — typed map-owned corridor materialization requests — carry authored corridor input networks into `DungeonMap.resolveCorridor(...)` and `DungeonMap.rehydrateCorridor(...)`.
