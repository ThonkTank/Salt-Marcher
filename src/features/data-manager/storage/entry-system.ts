...
export interface EntryManagerOptions<TEntry, TCategory extends string, TFilter extends string = never> {
  ...
  hideAddBar?: boolean;  // Hide the add bar for static entry lists
  hideActions?: boolean;  // Hide move/delete actions for static entries
}
...
export function mountEntryManager<TEntry, TCategory extends string, TFilter extends string = never>(
  ...
  // Only render add bar if not hidden
  ...
  const render = () => {
    ...
    entries.forEach((entry, index) => {
      ...
      // If hideActions is true, disable all action handlers
