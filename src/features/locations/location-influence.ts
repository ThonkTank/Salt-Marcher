...
import type { LocationData, LocationType } from "../../workmodes/library/locations/types";
...
/**
 * Get all hexes within influence area
 * @param area - Influence area
...
export function getInfluencedHexes(area: InfluenceArea): Array<{ hex: HexCoordinate; strength: number }> {
  ...
  // Iterate over bounding box around radius
  for (let q = area.center.q - area.radius; q <= area.center.q + area.radius; q++) {
    for (let r = area.center.r - area.radius; r <= area.center.r + area.radius; r++) {
      ...
      if (hexDistance(area.center, hex) <= area.radius) {
