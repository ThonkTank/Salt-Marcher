...
export class FactionHandler implements HookHandler {
    ...
    async execute(descriptor: HookDescriptor, context: HookExecutionContext): Promise<void> {
        ...
        logger.info("[faction-handler] Faction update triggered", {
        ...
        switch (config.action) {
            case "set_status":
                logger.info("[faction-handler] Would set faction status", {
            ...
            case "change_relationship":
                logger.info("[faction-handler] Would change faction relationship", {
                    faction: config.factionName,
            ...
            case "update_resources":
                logger.info("[faction-handler] Would update faction resources", {
                    faction: config.factionName,
            ...
            default:
                logger.warn("[faction-handler] Unknown action type", {
                    action: config.action,
                });
        }
    }
