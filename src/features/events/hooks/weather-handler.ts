...
export class WeatherHandler implements HookHandler {
    ...
    async execute(descriptor: HookDescriptor, context: HookExecutionContext): Promise<void> {
        ...
        logger.info("[weather-handler] Weather update triggered", {
        ...
        if (config.hexCoordinates) {
            logger.info("[weather-handler] Would update weather at hex", config.hexCoordinates);
        } else {
            logger.info("[weather-handler] Would update weather at current travel location");
        }
    }
}
