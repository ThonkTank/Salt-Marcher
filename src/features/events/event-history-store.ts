...
export class EventHistoryStore {
    ...
    addEvent(entry: TriggeredEventEntry): void {
        this.timeline.update((entries) => {
            ...
            logger.info("[event-history-store] Event added to timeline", {
                entryId: entry.id,
    ...
    addPhenomenon(entry: TriggeredPhenomenonEntry): void {
        this.timeline.update((entries) => {
            const updated = [...entries, entry];
            logger.info("[event-history-store] Phenomenon added to timeline", {
                entryId: entry.id,
    ...
    markAsRead(entryId: string): void {
        this.readEntries.update((entries) => {
            ...
            updated.add(entryId);
            logger.info("[event-history-store] Entry marked as read", { entryId });
    ...
    markAsUnread(entryId: string): void {
        this.readEntries.update((entries) => {
            ...
            logger.info("[event-history-store] Entry marked as unread", { entryId });
            this.saveToStorage(this.getCurrentTimeline(), updated);
            return updated;
        });
    }
    ...
    markAllAsRead(): void {
        ...
        logger.info("[event-history-store] All entries marked as read", {
    ...
    clear(): void {
        ...
        logger.info("[event-history-store] Timeline cleared");
    ...
    private saveToStorage(timeline: TimelineEntry[], readEntries: Set<string>): void {
        try {
            ...
            logger.error("[event-history-store] Failed to save to storage", error);
        }
    }
    ...
    private loadFromStorage(): void {
        try {
            ...
            if (data.version === 1) {
                ...
                logger.info("[event-history-store] Loaded from storage", {
                    timelineCount: data.timeline?.length || 0,
                    readCount: data.readEntries?.length || 0,
                });
            }
        } catch (error) {
            logger.error("[event-history-store] Failed to load from storage", error);
        }
    }
}
