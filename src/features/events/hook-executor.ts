...
import type { CalendarEvent, HookDescriptor } from "../../workmodes/almanac/domain";
import type { PhenomenonOccurrence } from "../../workmodes/almanac/domain";
...
class HookHandlerRegistry {
    ...
    register(handler: HookHandler): void {
        ...
        logger.info("[hook-executor] Registered hook handler", { type: handler.type });
    }
...
export class HookExecutor {
    ...
    async executeHooks(
        ...
        logger.info("[hook-executor] Executing hooks", {
    ...
    private async executeHook(descriptor: HookDescriptor, context: HookExecutionContext): Promise<void> {
        ...
        if (!handler) {
            logger.warn("[hook-executor] No handler registered for hook type", {
        ...
        if (!handler.canHandle(descriptor)) {
            logger.warn("[hook-executor] Handler cannot handle hook", {
                type: descriptor.type,
                hookId: descriptor.id,
            });
            return;
        ...
        try {
            logger.info("[hook-executor] Executing hook", {
                type: descriptor.type,
                hookId: descriptor.id,
            ...
            logger.info("[hook-executor] Hook executed successfully", {
                type: descriptor.type,
                hookId: descriptor.id,
            });
        } catch (error) {
            logger.error("[hook-executor] Hook execution failed", {
                type: descriptor.type,
                hookId: descriptor.id,
                error,
            });
