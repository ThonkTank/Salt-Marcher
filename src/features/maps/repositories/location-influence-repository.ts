...
import type { LocationData } from "../../../workmodes/library/locations/types";
        ...
        for (const file of locationFiles) {
            try {
                ...
                for (const { hex, strength } of influenced) {
                    // Convert cube coordinates to odd-r (TileCoord)
                    const tileCoord = cubeToOddr(hex.q, hex.r);
...
/**
 * Convert cube coordinates (q, r, s) to odd-r (TileCoord)
 */
function cubeToOddr(q: number, r: number): TileCoord {
    const col = q + (r - (r & 1)) / 2;
    const row = r;
    return { r: row, c: col };
}
