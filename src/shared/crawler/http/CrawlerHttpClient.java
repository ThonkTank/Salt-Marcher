...
final class CrawlerHttpClient {
    ...
    public static String get(String url, HttpClient client, String cobaltSession)
        ...
        return get(url, url, client, cobaltSession, 0);
    }
    ...
    private static String get(
        ...
        if (redirectCount > MAX_REDIRECTS) {
            throw new IOException("Too many redirects (>" + MAX_REDIRECTS + ") for: " + originalUrl);
        }
        ...
        if (status >= 300 && status < 400) {
            ...
            return get(originalUrl, redirectUri.toString(), client, cobaltSession, redirectCount + 1);
        }
        ...
        if (status == 404) {
            throw new IOException("Not found (HTTP 404): " + originalUrl);
        }
        if (status == 429) {
            throw new IOException("Rate limit reached (HTTP 429). Increase delay.ms?");
        }
        if (status != 200) {
            throw new IOException("Unexpected HTTP status " + status + " for: " + originalUrl);
        }
