...
/**
 * Owner root seam for the aggregated world travel host.
 */
public final class TravelObject {

    private final ComposeTraveltabInput.TraveltabInput traveltab;

    public TravelObject(ComposeTraveltabInput input) {
        ComposeTraveltabInput resolvedInput = java.util.Objects.requireNonNull(input, "input");
        this.traveltab = new TravelAssembly(resolvedInput).composeTraveltab();
    }
    ...
    private static final class TravelAssembly {
        ...
        private TravelAssembly(ComposeTraveltabInput input) {
            this.input = input;
        }
        ...
        private void activateMap(LoadMapsInput.MapSummary map) {
            ...
            hintLabel.setText("Kartenwahl und Lifecycle bleiben lokal im Travel-Tab.");
        ...
        private ComposeShellInput.SurfaceInput composeChildSurface(LoadMapsInput.MapSummary map) {
            if (map.ref().kind() instanceof LoadMapsInput.DungeonKind) {
                ComposeDungeontravelInput composeDungeontravelInput = new ComposeDungeontravelInput(map);
                return new DungeonObject().composeDungeontravel(composeDungeontravelInput).surface();
            }
            ComposeHextravelInput composeHextravelInput = new ComposeHextravelInput(map);
            return new HexObject().composeHextravel(composeHextravelInput).surface();
        }
        ...
        private void showCatalogState() {
            ...
            String statusBody = loadedMaps.errorMessage().isEmpty()
                    ? "Travel braucht mindestens eine Hexmap oder Dungeon-Karte aus der SQLite-DB."
            ...
            hintLabel.setText(loadedMaps.errorMessage().isEmpty()
                    ? "Sobald Karten vorhanden sind, bindet Travel den passenden Child-Host ein."
                    : "Travel zeigt den Fehler explizit an und faellt nicht still auf leere Inhalte zurueck.");
            ...
            setHostNode(mainHost, createHostPlaceholder("Travel", statusBody));
        ...
        private static String buildHintText(LoadMapsInput.LoadedMapsInput loadedMaps) {
            if (!loadedMaps.errorMessage().isEmpty()) {
                return "Travel konnte den Kartenkatalog nicht laden und zeigt den Fehlerzustand.";
            }
            if (loadedMaps.maps().isEmpty()) {
                return "Travel zeigt einen expliziten Leerzustand, bis Karten aus SQLite verfuegbar sind.";
            }
            return "Travel waehlt je nach Kartentyp automatisch den passenden Child-Host.";
        }

        private static String describeHost(LoadMapsInput.MapSummary map) {
            return map.ref().kind() instanceof LoadMapsInput.DungeonKind ? "Dungeon Travel" : "Hex Travel";
        }
