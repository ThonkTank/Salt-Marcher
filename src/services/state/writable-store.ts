...
export function derived<S extends ReadonlyArray<any>, T>(
        ...
        if (value !== newValue) {
            ...
            subscribers.forEach(subscriber => {
                try {
                    ...
                    logger.error("[DerivedStore] Subscriber error:", error);
                }
            });
        }
    };
    ...
    const subscribe = (subscriber: Subscriber<T>): Unsubscriber => {
        ...
        try {
            ...
            logger.error("[DerivedStore] Initial subscriber call error:", error);
        }
...
export type { ReadableStore } from "./store.interface";