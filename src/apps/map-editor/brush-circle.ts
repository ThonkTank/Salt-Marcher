...
export function attachBrushCircle(handles: Handles, opts: Opts): BrushCircleController {
    ...
    // Skalierung: pointy-top Hex → vStep = 1.5 * R
    const R = opts.hexRadiusPx;
    const vStep = 1.5 * R;
    const toPx = (d: number) => R + Math.max(0, d) * vStep; // Distanz 0 => R, 1 => 2.5R, 2 => 4R ...

    // Kreis
    const circle = document.createElementNS("http://www.w3.org/2000/svg", "circle");
    circle.setAttribute("cx", "0");
    circle.setAttribute("cy", "0");
    circle.setAttribute("r", String(toPx(opts.initialRadius))); // <- kein Runit mehr
    ...
    function updateRadius(hexDist: number) {
        circle.setAttribute("r", String(toPx(hexDist))); // <- kein Runit mehr
        bringToFront();
    }
