...
import { createSpellGroup } from "./components/spellcasting/spell-group";
import type { SpellInputHandle } from "./components/spellcasting/spell-input";
...
// mountSpellInput has been moved to components/spellcasting/spell-input.ts
...
export function mountCreatureSpellcastingSection(
  ...
  summarySetting.settingEl.addClass("sm-cc-setting");
  summarySetting.settingEl.addClass("sm-cc-setting--textarea");
  ...
  notesSetting.settingEl.addClass("sm-cc-setting");
  notesSetting.settingEl.addClass("sm-cc-setting--textarea");
  ...
  abilitySetting.settingEl.addClass("sm-cc-setting");
  abilitySetting.settingEl.addClass("sm-cc-spellcasting__ability");
  ...
  function renderGroups() {
    ...
    spellcasting.groups.forEach((group, index) => {
      const handles = createSpellGroup(groupsContainer, {
        group,
        groupIndex: index,
        totalGroups: spellcasting.groups.length,
        getCandidates: resolveAvailableSpells,
        onUpdate: () => {
          updatePreview(true);
        },
        onMove: (direction) => {
          if (direction === "up" && index > 0) {
            ...
            refreshGroups();
          } else if (direction === "down" && index < spellcasting.groups.length - 1) {
            const [item] = spellcasting.groups.splice(index, 1);
            spellcasting.groups.splice(index + 1, 0, item);
            refreshGroups();
          }
