...
export function mountCreatureBasicsSection(parent: HTMLElement, data: StatblockData) {
  ...
  const registerCell = (
    setting: Setting,
    layout: "full" | "half" | "quarter" = "half",
  ) => {
    setting.settingEl.classList.add("sm-cc-basics__cell");
    setting.settingEl.classList.add(`sm-cc-basics__cell--${layout}`);
  };

  const idSetting = new Setting(grid).setName("Name");
  registerCell(idSetting, "half");
  idSetting.addText((t) => {
  ...
  registerCell(alignSetting, "half");
  ...
  registerCell(speedSetting, "full");
  ...
  const mkStatSetting = (
    ...
    layout: "quarter" | "half" | "full" = "quarter",
  ) => {
    const setting = new Setting(grid).setName(label);
    registerCell(setting, layout);
    setting.addText((t) => {
  ...
  mkStatSetting("HP", "150", "hp", "quarter");
  mkStatSetting("AC", "17", "ac", "quarter");
  mkStatSetting("Init", "+7", "initiative", "quarter");
  mkStatSetting("PB", "+4", "pb", "quarter");
  mkStatSetting("HD", "20d10 + 40", "hitDice", "half");
  mkStatSetting("CR", "10", "cr", "quarter");
  mkStatSetting("XP", "5900", "xp", "quarter");
}
