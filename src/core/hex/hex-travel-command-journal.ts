...
import {
  ...
  type HexTravelCommandReceipt
} from '../../shared/contracts/hex-travel-command.js'
import { fingerprintExcluding } from '../fingerprint.js'

export function initializeHexTravelCommandJournal(
  database: Database.Database
): void {
  database.exec(`CREATE TABLE IF NOT EXISTS hex_travel_command_receipt (
...
/** Immutable original outcomes, committed in the same transaction as the action. */
export class HexTravelCommandJournal {
  constructor(private readonly database: Database.Database) {}
  read(input: HexTravelCommand): HexTravelCommandReceipt | null {
    const row = this.database
      .prepare(
        `SELECT request_fingerprint AS fingerprint,
      result_schema_version AS version, result_json AS result
      FROM hex_travel_command_receipt WHERE command_id = ?`
      )
    ...
    return hexTravelCommandReceiptSchema.parse(
      JSON.parse(row.result) as unknown
    )
  }
  record(input: HexTravelCommand, receipt: HexTravelCommandReceipt): void {
    this.database
      .prepare(
        `INSERT INTO hex_travel_command_receipt
      (command_id, request_fingerprint, result_schema_version, result_json) VALUES (?, ?, 1, ?)`
      )
      .run(
        input.commandId,
        fingerprintExcluding(input, ['commandId']),
        JSON.stringify(hexTravelCommandReceiptSchema.parse(receipt))
      )
  }
}
