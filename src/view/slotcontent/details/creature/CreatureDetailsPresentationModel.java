...
public final class CreatureDetailsPresentationModel {
    ...
    public CreatureDetailsPresentationModel(CreatureDetailResult result) {
    ...
    public void connect(
            ...
            Consumer<DetailState> detailSink
    ) {
        ...
        Consumer<DetailState> safeDetailSink = detailSink == null ? ignored -> { } : detailSink;
        ...
        load();
        DetailState current = detail.get();
        if (current != null) {
    ...
    private static List<ActionGroup> sections(CreatureDetail creature) {
        ...
        List<ActionLine> actions = creature.actions().stream()
                .map(CreatureDetailsPresentationModel::actionLine)
                .toList();
        if (!actions.isEmpty()) {
