...
final class SquareMapGridPainter {
    ...
    static void paintGrid(GraphicsContext graphics, MapViewport viewport) {
        ...
        for (int index = 0; index < SquareMapRenderTheme.GRID_STEPS.length; index++) {
            double pixelSpacing = scale * SquareMapRenderTheme.GRID_STEPS[index];
            if (pixelSpacing < 10.0) {
                continue;
            }
            paintGridTier(graphics, viewport, scale, SquareMapRenderTheme.GRID_STEPS[index], index);
        }
