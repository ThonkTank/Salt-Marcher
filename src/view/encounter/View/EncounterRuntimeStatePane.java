...
public final class EncounterRuntimeStatePane {
    ...
    public EncounterRuntimeStatePane(EncounterModel model, EncounterController controller) {
        this.model = Objects.requireNonNull(model, "model");
        Objects.requireNonNull(controller, "controller");
        content.getStyleClass().add("scene-pane");
        content.setPadding(new Insets(12));
        content.getChildren().setAll(
                summaryLabel(model.texts().lockSummaryProperty()),
                summaryLabel(model.texts().excludeSummaryProperty()),
                summaryLabel(model.texts().statusTextProperty()),
    ...
    private Button actionButton(String text, boolean requiresSelection, Runnable action) {
        ...
        if (requiresSelection) {
            button.disableProperty().bind(Bindings.isNull(model.alternatives().selectedAlternativeProperty()));
        }
        return button;
    }
}
