...
public final class EncounterCombatPartyMemberPopupStateView {

    private final AnchoredPopupView popup = new AnchoredPopupView();
    private Consumer<EncounterCombatPartyMemberPopupStateViewInputEvent> viewInputEventHandler = ignored -> { };

    public EncounterCombatPartyMemberPopupStateView() {
    }

    public void onViewInputEvent(Consumer<EncounterCombatPartyMemberPopupStateViewInputEvent> handler) {
        viewInputEventHandler = handler == null ? ignored -> { } : handler;
    }

    public void show(Node anchor, List<EncounterCombatPartyMemberButtonStateView.Candidate> candidates) {
        ...
        for (EncounterCombatPartyMemberButtonStateView.Candidate candidate : candidates) {
            ...
            Runnable apply = () -> {
                popup.hide();
                viewInputEventHandler.accept(new EncounterCombatPartyMemberPopupStateViewInputEvent(
                        candidate.memberId(),
                        parse(initiativeField.getText())));
            };
