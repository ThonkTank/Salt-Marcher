...
public final class CatalogPresentationModel {
    ...
    public CatalogPresentationModel() {
    ...
    SearchInput currentSearchInput() {
        ...
        return new SearchInput(filters, selectedSort, PAGE_SIZE, pageOffset);
    }
    ...
    private void applySearchResultInternal(CreatureCatalogPageResult result) {
        ...
        rows.setAll(page.rows().stream().map(CatalogPresentationModel::toCatalogRow).toList());
        updatePagingLabels();
    ...
    public record SearchInput(
            CreatureFilters filters,
            SortOption sortOption,
            int pageSize,
            int pageOffset
    ) {
        public SearchInput {
            filters = filters == null ? CreatureFilters.empty() : filters;
