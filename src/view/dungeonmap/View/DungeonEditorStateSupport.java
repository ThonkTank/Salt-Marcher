...
final class DungeonEditorStateSupport {
    ...
    static VBox mutationFeedbackCard(DungeonMapSurfaceViewModel controller) {
    ...
    private static VBox roomMoveControls(
        ...
        boolean canMoveRoom = controller.viewState().canApplyEditorOperation()
                && selectedTarget != null
                && "room".equalsIgnoreCase(selectedTarget.ownerKind());
        Button up = moveButton("Raum hoch", 0, -1, canMoveRoom, controller, selectedTarget, viewportSupplier);
        Button down = moveButton("Raum runter", 0, 1, canMoveRoom, controller, selectedTarget, viewportSupplier);
        Button left = moveButton("Raum links", -1, 0, canMoveRoom, controller, selectedTarget, viewportSupplier);
        Button right = moveButton("Raum rechts", 1, 0, canMoveRoom, controller, selectedTarget, viewportSupplier);
    ...
    private static Button moveButton(
        ...
        button.setOnAction(event -> controller.moveRoomAnchor(deltaQ, deltaR, selectedTarget, viewportSupplier.get()));
        return button;
    }
