...
public final class PartyRosterTopBarContentModel {
    ...
    void showPending(String status) {
        ...
        showPanel((current == null ? PanelContent.loadingContent() : current).withPending(status));
    }

    void showStatus(String status, boolean error) {
        PanelContent current = panelContent.get();
        showPanel((current == null ? PanelContent.loadingContent() : current).withStatus(status, error));
    }

    void showReadyStatus(String status, boolean error) {
        PanelContent current = panelContent.get();
        showPanel((current == null ? PanelContent.loadingContent() : current).withReadyStatus(status, error));
    }

    void showReserveSearch(String searchText) {
        PanelContent current = panelContent.get();
        showPanel((current == null ? PanelContent.loadingContent() : current).withReserveSearch(searchText));
    }

    String reserveSearchText() {
        PanelContent current = panelContent.get();
        return (current == null ? PanelContent.loadingContent() : current).reserveSearchText();
    }
    ...
    @Nullable MemberModel findMember(long memberId) {
        ...
        PanelContent current = panelContent.get();
        PanelContent safePanel = current == null ? PanelContent.loadingContent() : current;
    ...
    static final class MemberPresentation {

        private MemberPresentation() {
        }
