...
public final class PartyTopBarPresentationModel {
    ...
    public PartyTopBarPresentationModel() {
    }
    ...
    public ActionPlan addExisting(@Nullable Long id, String name) {
        if (id == null || id.longValue() <= 0) {
            return rejectedMutationPlan("Charakter konnte nicht gefunden werden.");
        }
        long characterId = id.longValue();
        String successMessage = displayName(name) + " wurde zur aktiven Party hinzugefuegt.";
        return submitMutation(
                new ActionIntent(ActionKind.SET_MEMBERSHIP, characterId, MembershipState.ACTIVE, null, 0, RestAction.NONE, successMessage),
                successMessage);
    }

    public ActionPlan createCharacter(CharacterDraftModel draft) {
        ...
        if (!parsedDraft.valid()) {
            return rejectedMutationPlan(parsedDraft.message());
        }
        String successMessage = displayName(parsedDraft.displayName()) + " wurde erstellt und zur Party hinzugefuegt.";
        return submitMutation(
                new ActionIntent(
                        ActionKind.CREATE_CHARACTER,
                        0L,
    ...
    public ActionPlan updateCharacter(CharacterDraftModel draft) {
        CharacterDraftModel safeDraft = draft == null ? CharacterDraftModel.empty() : draft;
        ...
        String successMessage = displayName(parsedDraft.displayName()) + " wurde gespeichert.";
        return submitMutation(
                new ActionIntent(
                        ActionKind.UPDATE_CHARACTER,
                        characterId,
                        MembershipState.ACTIVE,
                        Objects.requireNonNull(parsedDraft.draft()),
                        0,
    ...
    public ActionPlan deleteCharacter(@Nullable Long id, String name) {
        if (id == null || id.longValue() <= 0) {
            return rejectedMutationPlan("Charakter konnte nicht gefunden werden.");
        }
        long characterId = id.longValue();
        String successMessage = displayName(name) + " wurde geloescht.";
        return submitMutation(
                new ActionIntent(ActionKind.DELETE_CHARACTER, characterId, MembershipState.ACTIVE, null, 0, RestAction.NONE, successMessage),
                successMessage);
    }

    public ActionPlan removeFromParty(@Nullable Long id, String name) {
        if (id == null || id.longValue() <= 0) {
            return rejectedMutationPlan("Charakter konnte nicht gefunden werden.");
        }
        long characterId = id.longValue();
        String successMessage = displayName(name) + " wurde aus der aktiven Party entfernt.";
        return submitMutation(
                new ActionIntent(ActionKind.SET_MEMBERSHIP, characterId, MembershipState.RESERVE, null, 0, RestAction.NONE, successMessage),
                successMessage);
    }

    public ActionPlan adjustXp(@Nullable Long id, String name, int xpDelta) {
        if (xpDelta == 0) {
            return rejectedMutationPlan("XP-Korrektur darf nicht 0 sein.");
        }
        if (id == null || id.longValue() <= 0) {
            return rejectedMutationPlan("Charakter konnte nicht gefunden werden.");
        }
        ...
        return submitMutation(
                new ActionIntent(ActionKind.ADJUST_XP, characterId, MembershipState.ACTIVE, null, xpDelta, RestAction.NONE, successMessage),
                successMessage);
    }

    public ActionPlan shortRest() {
        return submitMutation(
                new ActionIntent(ActionKind.PERFORM_REST, 0L, MembershipState.ACTIVE, null, 0, RestAction.SHORT_REST,
                        "Short Rest wurde fuer die aktive Party ausgefuehrt."),
                "Short Rest wurde fuer die aktive Party ausgefuehrt.");
    }

    public ActionPlan longRest() {
        return submitMutation(
                new ActionIntent(ActionKind.PERFORM_REST, 0L, MembershipState.ACTIVE, null, 0, RestAction.LONG_REST,
    ...
    private ActionPlan submitMutation(ActionIntent intent, String successMessage) {
        if (mutationInFlight) {
            return rejectedMutationPlan("Party-Aktion laeuft bereits.");
        }
        mutationInFlight = true;
        panel.set(safePanel().withPending("Speichere..."));
        lastActionResult = ActionResult.pending(successMessage);
        return new ActionPlan(lastActionResult, intent);
    }
    ...
    private ActionPlan rejectedMutationPlan(String message) {
        showStatus(message, true);
        return new ActionPlan(ActionResult.failure(message), null);
    }
    ...
    public record ActionPlan(ActionResult result, @Nullable ActionIntent intent) {
    }
