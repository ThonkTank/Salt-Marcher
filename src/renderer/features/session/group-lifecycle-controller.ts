...
import type { GroupLifecyclePort } from './use-group-lifecycle-port.js'
...
export class GroupLifecycleController {
  ...
  private attempt: SceneGroupLifecycleCommand | null = null
  private unsaved: SceneGroupLifecycleCommand | null = null
  private completion: Completion | null = null
  private attached = true
  private unregister: (() => void) | null = null
  private readonly ownerId = `group-lifecycle-${crypto.randomUUID()}`
  ...
  execute = (input: SceneGroupLifecycleCommand): Promise<boolean> => {
    if (this.held() || this.state.conflict || !this.attached)
      return Promise.resolve(false)
    return this.start(input)
  }
  private start(input: SceneGroupLifecycleCommand): Promise<boolean> {
  ...
  settle = async (): Promise<boolean> => {
    ...
    return this.run(async () => {
      ...
      this.publish({
        ...
        conflict,
        error: message(
          conflict ? 'group.lifecycleConflict' : 'character.commandAbsent'
        )
      })
      return true
    })
  ...
  private matchesRevisions(
    ...
    const group = snapshot.scene.scenes
      .find((scene) => scene.id === command.sceneId)
      ?.groups.find((group) => group.id === command.groupId)
    return group?.revision === command.expectedGroupRevision
  }
  save = async (): Promise<boolean> => {
    if (!(await this.settle())) return false
    if (!this.unsaved) return true
    if (this.state.conflict) throw new Error(message('group.lifecycleConflict'))
