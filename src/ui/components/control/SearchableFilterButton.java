...
public class SearchableFilterButton extends Button {
    ...
    public SearchableFilterButton(String label, List<String> options,
        ...
        if (options.size() > SEARCH_FIELD_THRESHOLD) {
            ...
            searchField.textProperty().addListener((obs, o, n) -> filterCheckboxes(searchField.getText()));
            popupContent.getChildren().add(searchField);
            popup.setOnShown(e -> javafx.application.Platform.runLater(searchField::requestFocus));
        } else {
            popup.setOnShown(e -> javafx.application.Platform.runLater(popupContent::requestFocus));
        }
