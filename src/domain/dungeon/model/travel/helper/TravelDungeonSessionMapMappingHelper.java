...
public final class TravelDungeonSessionMapMappingHelper {

    private TravelDungeonSessionMapMappingHelper() {
    }
    ...
    public static MapData toInternalMap(@Nullable DungeonMapSnapshot map) {
        ...
        return new MapData(
                ...
                safeMap.width(),
                safeMap.height(),
                safeMap.areas().stream().map(TravelDungeonSessionMapMappingHelper::toInternalArea).toList(),
                safeMap.boundaries().stream().map(TravelDungeonSessionMapMappingHelper::toInternalBoundary).toList(),
                safeMap.features().stream().map(TravelDungeonSessionMapMappingHelper::toInternalFeature).toList());
    }
    ...
    private static AreaData toInternalArea(@Nullable DungeonAreaSnapshot area) {
        ...
        return new AreaData(
                ...
                area.id(),
                area.label(),
                area.cells().stream().map(TravelDungeonSessionMapMappingHelper::toInternalCell).toList());
    }
    ...
    private static FeatureData toInternalFeature(@Nullable DungeonFeatureSnapshot feature) {
        ...
        return new FeatureData(
                ...
                feature.id(),
                feature.label(),
                feature.cells().stream().map(TravelDungeonSessionMapMappingHelper::toInternalCell).toList(),
                feature.description(),
