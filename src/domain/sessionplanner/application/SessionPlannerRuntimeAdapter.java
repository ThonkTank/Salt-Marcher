...
public final class SessionPlannerRuntimeAdapter implements SessionPartyFactsLookup, SessionEncounterFactsLookup {

    private final PartyApplicationService party;
    private final EncounterApplicationService encounters;

    public SessionPlannerRuntimeAdapter(
    ...
    public ActivePartyMembersFact loadActivePartyMembers() {
        ...
        return new ActivePartyMembersFact(
                true,
                result.members().stream().map(SessionPlannerRuntimeAdapter::toPartyMemberFact).toList(),
                "");
    }
    ...
    public AdventuringDayFact calculateAdventuringDay(List<Integer> levels, int plannedEncounterXp) {
        ...
        if (result.status() != ReadStatus.SUCCESS || result.calculation() == null || result.calculation().budget() == null) {
    ...
    public EncounterPlanListFact listEncounterPlans() {
        ...
        return new EncounterPlanListFact(
                true,
                result.plans().stream().map(SessionPlannerRuntimeAdapter::toSavedEncounterPlanFact).toList(),
                "");
    }
    ...
    public EncounterPlanFact loadEncounterPlan(long encounterPlanId) {
        ...
        if (result.status() != EncounterPlanBudgetStatus.SUCCESS || result.summary() == null) {
            String message = result.message().isBlank() ? "Encounter-Plan konnte nicht geladen werden." : result.message();
