...
public final class SqliteEncounterLocalGateway {
    ...
    public static FeatureStoreDefinition storeDefinition() {
        EncounterSchemaInitializer schemaInitializer = new EncounterSchemaInitializer();
        SqliteSchemaValidator targetSchema = SqliteSchemaValidator.exactSchema(
                EncounterSchemaInitializer.CREATE_TABLE_SQL,
                EncounterSchemaInitializer.CREATE_INDEX_SQL,
                EncounterSchemaInitializer.OWNED_OBJECT_PREFIXES,
                List.of());
        return FeatureStoreDefinition.validated(
                OWNER, targetSchema,
                new SqliteMigration(1, schemaInitializer::initializeCurrentTarget));
    }
    ...
    public Optional<EncounterPlanSnapshotRecord> load(long planId) {
        try (Connection connection = openReadyConnection()) {
            ...
            return Optional.of(new EncounterPlanSnapshotRecord(
                    plan.get(),
                    creatures,
                    generatedBatches.loadOrigin(connection, planId)));
        } catch (SQLException exception) {
    ...
    private Optional<EncounterPlanSnapshotRecord> loadSaved(Connection connection, long planId) throws SQLException {
        ...
        List<EncounterPlanCreatureRecord> creatures = store.loadCreatures(connection, planId);
        return Optional.of(new EncounterPlanSnapshotRecord(
                plan.get(), creatures, generatedBatches.loadOrigin(connection, planId)));
    }
