...
import features.dungeon.adapter.javafx.map.DungeonMapContentModel.MapInteractionFrame;
...
final class DungeonMapFrameConsumption {
    ...
    EditorSurfaceFrame consumeEditorSurfaceFrame(
        ...
        if (editorSurfaceFrameCurrent
                && FrameChangeDetector.sameRenderRelevantSurface(safeSurfaceFrame, currentEditorSurfaceFrame)
                && FrameChangeDetector.sameRenderRelevantInteractionFrame(safeFrame, currentMapInteractionFrame)) {
    ...
    PointerTarget consumeRenderFrame(MapInteractionFrame interactionFrame) {
        MapInteractionFrame safeFrame = interactionFrame == null ? MapInteractionFrame.empty() : interactionFrame;
        Map<String, PointerTarget> pointerTargets = CellTargetResolution.withExactCellTargets(safeFrame.pointerTargets());
        currentHoverTarget = HoverTargetRetention.retainedHoverTarget(
                currentHoverTarget,
                pointerTargets,
                currentHoverContext,
    ...
    boolean updateHoverTarget(PointerTarget target) {
        ...
        if (HoverTargetRetention.sameHoverTarget(nextTarget, currentHoverTarget)) {
            return false;
        }
    ...
    private static final class HoverTargetRetention {
        ...
        private static boolean sameHoverTarget(PointerTarget first, PointerTarget second) {
            ...
            return switch (safeFirst.targetKind()) {
                ...
                case MARKER -> CellTargetResolution.sameCellOwner(safeFirst, safeSecond);
                case CELL -> sameCellTarget(safeFirst, safeSecond);
                case GRAPH_NODE -> CellTargetResolution.sameCellOwner(safeFirst, safeSecond);
        ...
        private static boolean sameCellTarget(PointerTarget first, PointerTarget second) {
            if (CellTargetResolution.exactCellAvailable(first, second)) {
                return CellTargetResolution.sameCellOwner(first, second)
                        && CellTargetResolution.sameCellRef(first.cellRef(), second.cellRef());
            }
            return CellTargetResolution.sameCellOwner(first, second);
        }
    ...
    private static final class CellTargetResolution {
    ...
    private static final class FrameChangeDetector {
