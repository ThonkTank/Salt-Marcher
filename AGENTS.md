...
6. Before editing a surface, read its owner doc and skill from the table below. If a surface has no clear owner, create a narrow documentation repair target instead of creating a second source of truth. Never omit, compress, or relocate documentation because of size; file a `doc-split` issue instead.
