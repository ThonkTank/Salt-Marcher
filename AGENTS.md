...
**Sensitive build-check surfaces:** `build.gradle.kts`, the repo-root `buildSrc` symlink, and `CODEOWNERS` are protected guardrail infrastructure. The authoritative `buildSrc` directory lives outside the repo under `~/Schreibtisch/SM/buildSrc`, and local Gradle verification is expected to fail if the repo stops pointing at that external location or if `build.gradle.kts` drifts from its external reference. Agents must not edit these files unless the user explicitly asks for build-check guardrail work or otherwise clearly approves those edits.
