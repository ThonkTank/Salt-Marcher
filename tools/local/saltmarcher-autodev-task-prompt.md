...
3a. Lies `AUTODEV_WORK_MIX_STATUS` am Sessionstart. Wenn die Zeile `work-mix: X% -> enforced` meldet, waehle keine meta-only Arbeit, ausser P0/P1-Regressionen, rote Required-Check-Reparaturen an offenen PRs, explizit owner-filed Issues oder RUNNER-DRIFT-Reparaturen bleiben uebrig. Migrationen zaehlen als product. Wenn nach der vollstaendigen Leiter nur meta-only Kandidaten uebrig bleiben, ist `no_work` als Ergebnis erlaubt.
...
```json
{
  ...
  "pr_number": 123,
  "result": "pr_opened|pr_open_red|merged|blocked|queue_done|no_work|dry_run_ok|backoff|timeout|crash",
  "red_checks": [],
...
`queue_done` ist nur gueltig, wenn eine Queue-Aufgabe mit Done-Sentinel abgeschlossen wurde. `no_work` ist nur gueltig, wenn `AUTODEV_WORK_MIX_STATUS` enforced meldet und nach der vollstaendigen Leiter nur meta-only Kandidaten uebrig sind. `dry_run_ok`, `backoff`, `timeout` und `crash` sind technische Wrapper-Resultate; normale Codex-Sessions verwenden sie nicht. `task_source=none` ist nur fuer technische Wrapper-Resultate oder RQ-2-`no_work` erlaubt. `blocked` ist kein Owner-Wartezustand; es benennt genau einen naechsten Reparaturgrund oder einen echten Provider-/Kosten-Backoff.
