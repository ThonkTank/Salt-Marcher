...
@BugPattern(
        ...
        summary = "Presentation-state and presentation-model types must live in ViewModel or exported api packages.",
...
public final class ViewModelOwnershipNamingChecker extends BugChecker implements BugChecker.ClassTreeMatcher {
    ...
    public Description matchClass(ClassTree tree, VisitorState state) {
        ...
        if (!packageName.startsWith("src.view.")
                ...
                || ViewArchitectureSupport.API_PACKAGE.matcher(packageName).matches()) {
            return Description.NO_MATCH;
        }
        ...
        return buildDescription(tree)
                .setMessage("Presentation-owned type '" + simpleName
                        + "' must live in ViewModel/ or an intentional api/ boundary, not in package '"
                        + packageName + "'.")
                .build();
    }
}
