...
@BugPattern(
        name = "ViewAssemblyDependencies",
        summary = "Assembly packages may only compose own MVVM buckets, foreign api, and domain boundaries.",
        severity = BugPattern.SeverityLevel.ERROR)
public final class ViewAssemblyDependencyChecker extends BugChecker
        implements BugChecker.CompilationUnitTreeMatcher {

    @Override
    public Description matchCompilationUnit(CompilationUnitTree tree, VisitorState state) {
        String packageName = ViewArchitectureSupport.packageName(tree);
        var matcher = ViewArchitectureSupport.ASSEMBLY_PACKAGE.matcher(packageName);
        if (!matcher.matches()) {
    ...
    private static boolean isForbiddenReference(String referencedType, String component) {
        ...
        if (viewType.component().equals(component)) {
            return !Set.of("assembly", "View", "ViewModel", "api").contains(viewType.bucket());
        }
