...
public final class DataRepositorySourceMechanicsRule extends AbstractJavaRule {
    ...
    public Object visit(ASTCompilationUnit node, Object data) {
        ...
        if (!sourceFacts.isUnderMainSourceRoots() || !isRepository(sourceFacts)) {
            return data;
        }

        String sanitizedSourceText = codeTextWithoutCommentsAndStrings(sourceFacts.text());
        for (String token : CONCRETE_SOURCE_TOKENS) {
            if (sanitizedSourceText.contains(token)) {
                asCtx(data).addViolationWithMessage(node,
                        "Data repository/ code must not own concrete source mechanics such as '"
