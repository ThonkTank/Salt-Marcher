...
description: Use before planning, implementing, refactoring, or reviewing anything under `src/view/**`, shell-facing contribution registration, Binders, `ContributionModel`, `ContentModel`, or `ContentPartModel` roles, optional `IntentHandler` roles, passive Views, reusable `slotcontent`, or adjacent `UI.md`. The canonical source of truth is `docs/project/architecture/patterns/view-layer.md`.
---
...
- reusable `slotcontent/**` keeps one closed reusable-unit shape only:
  exactly one `*View.java`, exactly one same-stem `*ContentModel.java`,
  optional owned `*ContentPartModel.java` submodels, and a same-stem
  `*ViewInputEvent.java` only when that View is interactive
...
- keep component-specific presentation state and component-specific
  presentation logic in the View's own same-stem `*ContentModel` or an owned
  same-unit `*ContentPartModel`
...
1. Assign every touched type one role: shell host, UI contribution, Binder,
   `ContributionModel`, `ContentModel`, `ContentPartModel`, optional
   `IntentHandler`, passive View, domain Model, or data adapter.
...
5. For every touched projection model, identify whether it is the active-root
   aggregate `*ContributionModel`, the same-stem `*ContentModel` for exactly
   one passive View, or an owned same-unit `*ContentPartModel`, and which exact
   surface state slice it owns.
...
8. Before smell- or size-driven refactors, ask whether the passive `View` is
   ...
   `ContentModel`, `ContentPartModel`, or upstream `published/*Model` readback
   path.
9. Move ambiguous logic to the owning role instead of copying nearby legacy
   placement.
10. Only after that ownership check may you use owned `*ContentPartModel`
    submodels or nested/private helper types as a local cleanup tactic; do not
    default to generic top-level helper files under `src/view/**`.
...
3. Check that bindable projection state and projection logic stay in the
   owning `ContributionModel`, `ContentModel`, or `ContentPartModel`.
...
9. Treat PMD-driven helper splits as findings when the real missing owner is
   the owning `ContributionModel`, `ContentModel`, `ContentPartModel`, or the
   upstream readback path.
10. Treat any top-level file under reusable `slotcontent/**` outside the
    closed shape `*View`, `*ViewInputEvent`, `*ContentModel`, and owned
    `*ContentPartModel` as a finding.
...
- If code decides what should be shown, enabled, selected, labelled, loaded,
  ...
  `ContributionModel`, `ContentModel`, or a bounded `ContentPartModel`.
...
- If code reacts to thin same-context `published/*Model` readback to refresh
  ...
  `ContentModel`, or `ContentPartModel`, not as a Binder-owned
  projection-forwarding workflow.
...
- If code prepares render-ready, hit-ready, label, geometry, selection, or
  ...
  same-stem `ContentModel`, owned `ContentPartModel`, or upstream read-side
  projection rather than in new standalone top-level helper files.
