...
cluster, wall, corridor, or stair catalogs.
...
| `DE-HANDLE-002` | Shared handle hit route | `DungeonMapView` hit testing over each handle type | `F16_HANDLE_VARIETY` | Each rendered handle resolves to a handle target, not to a presentation-only label or generic area. | Harness Gap |
...
| `DE-HANDLE-004` | Non-obstructive visual style | Render scene inspection | `F16_HANDLE_VARIETY` | Handles render smaller, rounder, and less obstructive than current label-like controls while remaining hittable. | Implementation Gap |
