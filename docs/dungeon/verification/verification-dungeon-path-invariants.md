...
| `DGI-PATH-006` | Pending production wall-draft path owner | Wall-create drafts can include intermediate points before final commit. | Deterministic segment accumulation is a target path/draft-owner obligation; local list mechanics alone do not qualify owner proof. | Harness must exercise the production draft/path owner API for start, intermediate point, completion candidate, and cancel facts. | Partial mechanics proof in `PathInvariantHarness`; production owner/API proof pending. | Wall owner owns final wall/open mutation. |
