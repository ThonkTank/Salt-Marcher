...
| --- | --- | --- | --- | --- | --- | --- |
| `DGI-TRANSITION-001` | `Transition` | Transition is an authored component with stable local facts. | A transition owns id, map id, one `TransitionAnchor` (`NONE`, `CELL`, or `EDGE`), description, destination, and optional local link state with normalized defaults. | Harness proves null/invalid normalization, anchor normalization, placed-state, derived display/travel cell behavior, description update, and destination replacement. | Qualified by `OwnerSuite=TransitionInvariantHarness`. | Edge authoring UI. |
...
| `DGI-TRANSITION-005` | Runtime travel projection over transition facts | Runtime may project transitions but must not own authored transition truth. | Travel transition targets are derived from core transition facts and stored only as runtime projection/session state; unlinked entrances stay visible with `Kein Ziel verknuepft` and fail movement through the missing-destination path without saving dungeon or overworld movement. | Harness proves travel actions, transition targets, unlinked placeholder action projection, blocked movement, and no saved movement from authored transition facts without persisting runtime transition truth. | Qualified by `OwnerSuite=RuntimeProjectionInvariantHarness`. | Full travel UI behavior. |
