...
| --- | --- | --- | --- | --- | --- | --- |
| `DGI-TRANSITION-001` | `Transition` | Transition is an authored component with stable local facts. | A transition owns id, map id, anchor cell, description, destination, and optional local link state with normalized defaults. | Harness proves null/invalid normalization, placed-state, description update, and destination replacement. | Partial mechanics proof by current `DGI-STR-007`; target transition owner unqualified. | Persistence row mapping. |
| `DGI-TRANSITION-002` | Bounded transition collection | Link behavior can span multiple transitions. | Link replacement, reverse-link creation, reverse-link cleanup, and reference checks are owned by a transition collection scoped to the authored map set being mutated. | Harness proves one-way and bidirectional link replacement plus cleanup of stale reverse links. | Partial mechanics proof by current `DGI-STR-007`; target transition collection owner unqualified. | Cross-map repository transaction shape. |
