...
| `DE-LABEL-004` | Room label derives from room floor cells | Render and hit room label | `F15_COMPLEX_CLUSTER` | Room label renders as subdued floor text from model-owned room floor cells with view-owned longest-wall orientation, rotated hit geometry, and matching inline-editor presentation. | Ready |
