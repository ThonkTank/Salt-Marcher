...
| `DE-FEATURE-003` | Create POI marker | Feature family default plus primary map click | `F6_MULTI_LEVEL_FLOORS` | One `dungeon_feature_markers` row, one `FEATURE_MARKER` topology ref, published POI feature, active-level render cell, and marker glyph persist at the clicked cell. | Ready |
| `DE-FEATURE-004` | Feature marker selection and hit identity | Feature create readback plus authored marker-cell hit | `F6_MULTI_LEVEL_FLOORS` | The authored marker cell resolves as `CELL` with element kind `FEATURE_MARKER`, not `HANDLE`; feature-marker create selects the marker and publishes inspector readback through state and map-surface snapshots. | Ready |
