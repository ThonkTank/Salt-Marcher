...
| `DE-COR-016` | Move cluster with dependent corridor route | Select tool plus whole-cluster drag where the moved cluster owns a referenced corridor anchor | `F5_CORRIDOR_WITH_ANCHOR` variant | Moving the cluster updates the dependent corridor route through SQLite, published snapshot, render, and reload while preserving dependent endpoint refs and omitting stale former route cells. | Ready |
