...
| `DE-COR-014` | Horizontal-blocked corridor uses fallback route | Corridor family plus two door hits where horizontal-first crosses a blocking room and vertical-first is open | `F17_VERTICAL_FALLBACK_CORRIDOR_ROUTE` | The full click route persists one corridor using the vertical-first fallback cells, reloads the same rendered route, and leaves the visible fallback corridor body as a semantic corridor target. | Ready |
