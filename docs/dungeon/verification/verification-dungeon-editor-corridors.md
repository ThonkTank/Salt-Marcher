...
| `DE-COR-013` | Generic endpoint materializes only at full commit | Pending corridor draft using generic room or corridor hit | `F12_ROOM_TO_DOOR_ROUTE_TARGET`, `F5_CORRIDOR_WITH_ANCHOR`, `F11_BLOCKED_CORRIDOR_ROUTE` | Generic room hits materialize the facing door only at successful commit; generic corridor hits reuse an existing anchor or create exactly one missing anchor only at successful commit; blocked generic completion materializes no endpoint. Success is proved through SQLite readback, topology identity, published snapshot, reload, and render. | Ready |
| `DE-COR-014` | Horizontal-blocked corridor uses fallback route | Corridor family plus two door hits where horizontal-first crosses a blocking room and vertical-first is open | `F17_VERTICAL_FALLBACK_CORRIDOR_ROUTE` | The first explicit door click persists no partial corridor; the full click route reuses existing explicit door endpoint identities, persists one corridor using the vertical-first fallback cells, reloads the same rendered route, and leaves the visible fallback corridor body as a semantic corridor target. | Ready |
