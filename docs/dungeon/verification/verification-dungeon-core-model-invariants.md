...
| ID | OwnerSuite | Scope | Expected invariant |
| --- | --- | --- | --- |
| `DGI-STR-001` | `DungeonStructureInvariantHarness` | `core/structure/CorridorRoomSet`, `CorridorBindings`, `CorridorAnchorSnap` | Corridor structure owns room-set normalization/removal, binding container rules, door removal by room, anchor binding/ref replacement by local anchor id, anchor snapping to the nearest host cell with level/row/column tie-breaks and fallback behavior, and target-local waypoint or anchor-ref removal. Topology-ref identity, persistence/readback, map-level route repair/deletion, and runtime graph ownership remain outside this model-invariant row and require RealRoute `DE-*` proof or future core topology/graph proof. |
