...
| `DGI-CMP-003` | `DungeonComponentInvariantHarness` | `core/component/CorridorDoorBinding`, `CorridorWaypoint`, `CorridorAnchorRef` | Corridor binding components keep local door, waypoint, and anchor-reference values. Topology refs remain boundary readback and persistence language, not component-owned mutable anchor identity. |
...
| `DGI-STR-002` | `DungeonStructureInvariantHarness` | `core/structure/CorridorBindingState`, `CorridorBindings` | Corridor binding state stores core `CorridorAnchor` values directly and replaces, references, and deduplicates anchors by host/local anchor identity without topology-ref bridge behavior. |
| `DGI-STR-003` | `DungeonStructureInvariantHarness` | `core/structure/CorridorRoutePlan` | Corridor structure owns interior route-anchor selection and waypoint planning for route splits. Planned anchor refs preserve selected core host/local anchor identity. |
| `DGI-STR-004` | `DungeonStructureInvariantHarness` | `core/structure/Corridor` | Corridor structure owns target-local door, anchor, and waypoint delete behavior, including room removal, route-waypoint pruning, anchor-ref removal, and waypoint removal. |
| `DGI-STR-005` | `DungeonStructureInvariantHarness` | `core/structure/CorridorNetwork` | Corridor network owns protected whole-corridor delete and detached-anchor pruning across authored corridors by core host/local anchor identity. |
