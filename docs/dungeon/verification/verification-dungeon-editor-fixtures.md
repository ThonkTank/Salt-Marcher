...
| `F1_SINGLE_ROOM` | Room, wall, door, narration, and selection baseline | Seeded room `R1` in cluster `C1` at level `0`; room anchor and cluster center `(2,2,0)` intentionally describe the fixture, not necessarily the current first-cell anchor chosen by freshly painted rooms; target authored floor storage is the cluster floor-cell set `(1..3, 1..3, 0)`; target authored wall storage is perimeter `WALL` boundary rows read back as north/east/south/west walls; any retained `dungeon_room_cluster_vertices.relative_x/relative_y` rows are legacy compatibility input only and must not be treated as target room-shape authority; no doors. |
