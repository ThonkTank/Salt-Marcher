...
| `F1_SINGLE_ROOM` | Room, wall, door, narration, and selection baseline | Seeded room `R1` in cluster `C1` at level `0`; room anchor and cluster center `(2,2,0)` intentionally describe the fixture, not necessarily the current first-cell anchor chosen by freshly painted rooms; target authored floor storage is the cluster floor-cell set `(1..3, 1..3, 0)`; target authored wall storage is perimeter `WALL` boundary rows read back as north/east/south/west walls; no doors. |
...
| `F14_LARGE_CURRENT_GEOMETRY_MAP` | Startup/input responsiveness for large persisted room geometry | One persisted map containing one room whose cluster stores at least 56,000 authored floor-cell rows in `dungeon_room_cluster_floor_cells` plus explicit perimeter `WALL` rows in `dungeon_room_cluster_edges`; the fixture represents current room-cluster geometry persistence and must not depend on retired vertex-table compatibility loading. Historical legacy-vertex datasets are manual migration inputs, not active behavior proof fixtures. |
| `F15_COMPLEX_CLUSTER` | True-corner, wall-run, and label placement behavior | One non-rectangular cluster `C1` at level `0` containing at least two rooms and an authored concave perimeter; the perimeter has at least six real wall corners, one interior wall run, one exterior wall run, and straight runs long enough to publish midpoint wall-run handles. Room labels are present as floor-cell-derived text with view-owned longest-wall orientation. The focused T-split UI fixture adds an authored vertical interior run crossed by a horizontal branch so `DE-CLUSTER-002` proves exact split-run `sourceEdges` and connector preservation through release/reload. |
| `F16_HANDLE_VARIETY` | Shared handle identity, selected cluster hit route, passive refs, preview, and visual style | One map combining `F15_COMPLEX_CLUSTER`, corridor `K1` with anchor `A1`, stair `S1`, and door `D1`, so the common handle model can cover selected cluster-corner publication/rendering alongside corridor, stair, and door handle publication. Door and corridor refs remain published facts but do not become generic canvas drag handles. True-corner drag readiness is owned and proved by the Ready `DE-CLUSTER-003` row; wall-run drag has boundary-stretch preview, exact selected `sourceEdges`, commit, reload, connector preservation, and invalid-rejection coverage in the Ready `DE-CLUSTER-002` row. `CLUSTER_WALL_RUN` is a selected-only smaller midpoint affordance. |
...
| `F18_OFFSET_MULTI_LEVEL_ROOMS` | Image-diff sensitivity across projection levels | Room `R1` cells `(1..3,1..3,0)` and room `R2` cells `(6..8,1..3,1)` on the same map. The offset geometry exists so image snapshot parity checks can distinguish visible projection-level render changes from routes whose current old-structure pixels remain unchanged. |
