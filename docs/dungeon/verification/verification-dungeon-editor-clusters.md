...
| `DE-CLUSTER-001` | Publish complex cluster corner and wall-run handles | Snapshot publication after loading a non-rectangular cluster | `F15_COMPLEX_CLUSTER` | Interactive handles appear on every authored wall corner and every non-trivial straight wall-run midpoint, inside and outside, with smaller wall-run marker styling. | Ready |
| `DE-CLUSTER-002` | Stretch selected true wall run | Select tool plus drag on a wall-midpoint handle | `F15_COMPLEX_CLUSTER` | Wall-run drag publishes preview, persists a valid move, preserves cluster and room identity, reloads, and leaves no duplicate or orphan wall rows. Invalid geometry rejection remains unqualified. | Partial |
