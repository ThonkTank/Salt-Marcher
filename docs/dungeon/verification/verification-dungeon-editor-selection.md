...
| `DE-HOVER-002` | Empty selection hover stays view-local | `OwnerSuite=DungeonEditorSelectionHarness`; `ProofType=RealRoute`; `Auswahl` plus repeated `DungeonMapView` mouse move over empty/off-grid map space | mixed route setup using empty/off-grid selection hover with corridor-anchor fixture context | No full editor snapshot publication, no runtime-frame publication, no canvas or hover-state publication, no synthetic hover overlay, and authored rows unchanged. | Ready |
