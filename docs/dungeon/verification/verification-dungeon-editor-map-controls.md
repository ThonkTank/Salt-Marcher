...
| `DE-START-001` | Startup remains responsive with large persisted vertex loops | Discovered shell startup and map key route | `F14_LARGE_STORED_VERTEX_MAP` | Startup lists but does not auto-load the large map, stays within deterministic shell latency, keeps vertex rows intact, and processes `Esc` within key-route latency. | Ready |
