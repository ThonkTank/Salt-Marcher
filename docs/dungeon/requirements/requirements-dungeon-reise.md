...
Source of Truth: Compact dungeon-facing `Reise` state-surface behavior, visible
states, and acceptance criteria.

# Dungeon Reise Requirements

## Goal

Define the compact read-mostly dungeon `Reise` surface that communicates
current dungeon travel context without replacing the full interactive dungeon
travel workspace.
...
- SaltMarcher currently shows a generic `Reise` placeholder in the global
  state-tab strip.
...
- when the active party is inside a dungeon, `Reise` MUST show dungeon travel
  context rather than a generic overworld placeholder
...
- if the party is not in a dungeon, the dungeon-specific `Reise` state MUST
  fall back to an explicit empty or non-applicable state
...
- The surface stays visibly smaller and less interactive than the full dungeon
  travel workspace.
- Dungeon `Reise` content changes when the underlying dungeon runtime state
  changes.
