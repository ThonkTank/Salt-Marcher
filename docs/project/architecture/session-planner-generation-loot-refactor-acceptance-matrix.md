...
| SGL-57 | Aggregate-owned operation fragments are the sole declarations; `operations.ts` composes their namespace/method, handler owner, mode, role, deadline, diagnostics, and input/output schemas into the exact Core and Main registries. | Shared boundary | Operation registry | Registry metadata tests | DONE |
