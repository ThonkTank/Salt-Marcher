...
| `view-binder-readback-source-is-domain-model-handle` | every Binder-owned domain-state intake path in an active root | none | none | Domain-backed view state is consumed only from a same-context read-side `published/*Model` handle loaded from the root `*ApplicationService`, then read only through `current()` and `subscribe(...)`. Direct one-shot `*Result`, `*Snapshot`, `*Payload`, `*SearchResult`, `*CalculationResult`, or `*Preview` responses are not used as authoritative view-state sources. |
