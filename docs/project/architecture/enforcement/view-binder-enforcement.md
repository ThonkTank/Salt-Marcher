...
- `./gradlew checkViewEnforcement --rerun-tasks --console=plain`
  runs the merged View compile-bound bundle, including the Binder-focused
  Error Prone checks. Canonical compile-side blocking behavior remains at
  `./gradlew compileJava`; aggregate blocking behavior enters
  `./gradlew checkArchitecture` and `./gradlew check` through the merged View
  bundle.
...
| `view-binder-same-root-runtime-role-surface` | Enforced | every active-root `*Binder.java` under `src/view/**` | Error Prone `ViewBinderDependencyBoundary` | `./gradlew compileJava` and `./gradlew checkViewEnforcement` | A Binder may depend on its own same-root `*ContributionModel`, optional same-root `*IntentHandler`, same-root passive `*View`, and same-root `*ViewInputEvent` types. Same-root `*PublishedEvent` is not part of the legal Binder role surface. |
