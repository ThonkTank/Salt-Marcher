...
| `view-binder-reusable-slotcontent-surface` | Enforced | every active-root `*Binder.java` under `src/view/**` that reuses `slotcontent/**` | Error Prone `ViewBinderDependencyBoundary` and jQAssistant `saltmarcher:ViewBinderDependencies` | `./gradlew compileJava` and `./gradlew checkViewBinderEnforcement` | The target reusable-slotcontent surface for a Binder is `*View`, `*ContentModel`, and reusable `*ViewInputEvent`. The current mechanical gate still reflects the older model and therefore admits `*InspectorEntry` and primitive support-value seams while forbidding direct reusable `*ViewInputEvent` dependencies; that is current drift, not target truth. |
