...
| --- | --- | --- | --- | --- | --- |
| `domain-applicationservice-root-presence` | Enforced | every active domain context under `src/domain/**` | domain-application-service bundle build-harness `DomainApplicationServiceTopologyRules` and `DomainApplicationServiceDocumentationRules` | `./gradlew checkArchitecture`, `./gradlew checkDocumentationEnforcement`, and `./gradlew checkDomainEnforcement` | Each active domain context contains one or more direct root boundary files whose names end with `ApplicationService.java`, and every `Application Service:` marker declared in `DOMAIN.md` resolves to a real direct root file. |
