...
| `domain-applicationservice-query-returns-read-model-handle` | Enforced | every public or protected non-constructor root `ApplicationService` method whose parameter carrier ends with `Query` | domain-application-service bundle Error Prone `DomainApplicationServiceApiShape` | `./gradlew compileJava` and `./gradlew checkDomainApplicationServiceEnforcement` | Root query methods return exactly one same-context read-side `published/*Model` handle directly. |
| `domain-applicationservice-command-no-direct-return` | Enforced | every public or protected non-constructor root `ApplicationService` method whose parameter carrier ends with `Command` | domain-application-service bundle Error Prone `DomainApplicationServiceApiShape` | `./gradlew compileJava` and `./gradlew checkDomainApplicationServiceEnforcement` | Root command methods return `void`; mutation feedback does not travel back as a direct root return value. |
