...
| `domain-applicationservice-role-reference-boundary` | Enforced | every root `*ApplicationService.java` under `src/domain/**` | domain-application-service bundle Error Prone `DomainApplicationServiceRoleBoundary` | `./gradlew compileJava` and `./gradlew checkDomainEnforcement` | Root services reference only same-context root or model-local `*UseCase` types and same-context `published/**` non-`*Model` carriers. Published carriers appear only as the single parameter of a public or protected root method; foreign root services, ports, repositories, models, helpers, callbacks, and outer-layer types are not legal root-service references. |
