...
| `domain-applicationservice-role-reference-boundary` | Enforced | every root `*ApplicationService.java` under `src/domain/**` | domain-application-service bundle Error Prone `DomainApplicationServiceRoleBoundary` | `./gradlew compileJava` and `./gradlew checkDomainEnforcement` | Root services reference only same-context root or model-local `*UseCase` types and same-context `published/*Command` carriers. The detailed carrier-position rule below defines the boundary-adapter exception and forbidden positions. Foreign root services, ports, repositories, models, helpers, callbacks, and outer-layer types are not legal root-service references. |
| `domain-applicationservice-thin-router-one-usecase-call` | Enforced | every public root `ApplicationService` command method | domain-application-service bundle Error Prone `DomainApplicationServiceThinRouter` | `./gradlew compileJava` and `./gradlew checkDomainEnforcement` | Each public root command method calls exactly one same-context `UseCase` method. Calls to nested UseCase-owned input carriers, factories, or enum normalizers do not count as separate UseCase operations. Branching may choose command data, but it must not become a multi-operation use-case dispatcher. |
...
#### Root Boundary Carrier Positions

A same-context `published/*Command` carrier is legal only as the single direct
parameter of a public or protected root method. That method may null-check the
parameter and read accessors to translate the boundary command into UseCase
arguments.

A private static one-argument `to*` or `from*` boundary adapter may accept one
direct same-context `published/*Command` carrier only to flatten that carrier
into UseCase-owned input. Same-context published non-`*Model` value carriers may
exist only as read-only locals rooted in that legal boundary command; published
command locals remain illegal.

Published carriers are otherwise illegal as fields, constructor parameters,
private/package method parameters, return types, local caches, class/interface
generic arguments, type-variable bounds, supertype generic arguments,
class-literal/type-uses, or arguments passed deeper into UseCase/helper/factory
work.
