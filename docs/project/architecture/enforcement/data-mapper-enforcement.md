...
- `./gradlew checkDataEnforcement --rerun-tasks --console=plain`
  runs the currently active Data Mapper-focused enforcement-documentation
  coverage checks through one root task.
...
| Invariant ID | Status | Applies When | Mechanical Owner | Diagnostic/Mechanical Route | What It Proves |
| --- | --- | --- | --- | --- | --- |
| `data-mapper-no-source-mechanics` | Review-Owned | every Java type under `src/data/**/mapper/` | none | none | `mapper/` code should not reference narrow concrete source APIs directly, but no active PMD, Error Prone, ArchUnit, jQAssistant, or build-harness rule currently blocks this exact source-pattern claim. |
