...
| --- | --- | --- | --- | --- | --- |
| `view-contributionmodel-active-root-role-shape` | Candidate | every aggregate projection-model role file in an active root under `src/view/leftbartabs/**`, `src/view/statetabs/**`, or `src/view/dropdowns/**` | build-harness `ViewLayerTopologyRules` | none yet | Active roots use exactly one aggregate projection-model role named `*ContributionModel.java`; active-root `*ContentModel.java` files are legal only as same-stem passive-View content models and are not alternate aggregate models. Current gates still need to admit those paired active-root `*ContentModel.java` files. |
...
| `view-contributionmodel-no-foreign-view-role-dependencies` | Candidate | every active-root `*ContributionModel.java` under `src/view/**` | planned update to Error Prone `ViewContributionModelDependencyBoundary` | none yet | A `ContributionModel` may coordinate same-root or intentionally reused child `*ContentModel` surfaces, but it does not depend on `*Contribution`, `*Binder`, `*IntentHandler`, `*View`, `*ViewInputEvent`, `*PublishedEvent`, `*InspectorEntry`, foreign `*ContentModel`, or other foreign view-unit role families. |
