...
| `view-contributionmodel-no-outward-request-or-publish-protocols` | Enforced | every active-root `*ContributionModel.java` under `src/view/**` | Error Prone `ViewContributionModelRequestProtocol` | `./gradlew compileJava` | A `ContributionModel` does not expose outward request-token or publish-like protocol members such as `*Request*Property()`, `*TokenProperty()`, or non-private `publish*()` seams. Binder-facing outward work leaves the local unit only through Binder-installed `Consumer<SameRootPublishedEvent>` seams on the local `IntentHandler`. |
