...
| --- | --- | --- | --- | --- | --- |
| `view-contributionmodel-active-root-role-shape` | Enforced | every projection-model role file in an active root under `src/view/leftbartabs/**`, `src/view/statetabs/**`, or `src/view/dropdowns/**` | build-harness `ViewLayerTopologyRules` | `./gradlew checkViewEnforcement` and `./gradlew checkViewEnforcement` | Active roots use the active projection-model role shape `*ContributionModel.java` rather than reusable `*ContentModel.java` or legacy `*ViewModel.java`, `*PresentationModel.java`, or `*Projector.java` files. |
