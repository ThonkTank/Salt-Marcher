...
| `domain-repository-no-same-context-published-state-channel` | Enforced | every repository under `src/domain/**` | domain-repository bundle Error Prone `DomainRepositoryPublishedStateBoundary` | `./gradlew compileJava` and `./gradlew checkDomainEnforcement` | Generic repositories do not replace same-context `published/*Model` readback with `publish*` methods, and no repository, including `*PublishedStateRepository`, may use `Object` payload channels. A simple name ending `PublishedStateRepository` may expose typed `publish*` methods as the canonical same-context publication sink. |
