...
| `layering-repository-src-direct-child-allowlist` | Enforced | every non-empty direct child bucket under `src/` | `layering-architecture-enforcement` bundle build-harness `LayeringArchitectureTopologyRules` | `./gradlew checkLayeringEnforcement` | `src/` contains only `features/`, `view/`, `domain/`, and `data/` as active direct layer roots. |
...
| `layering-bootstrap-registration-order-and-generic-discovery-path` | Review-Owned | every feature registration path from startup into runtime layer composition | none | none | Current cross-layer startup composition follows the documented path: bootstrap discovers data `*ServiceContribution` roots, populates the shared `ServiceRegistry`, constructs the shell, and then discovers view `*Contribution` roots without feature-specific bootstrap registries. Target migrated feature shell bindings register only through the feature-runtime shell seam. Current blockers constrain pieces of that path, but not the whole sequencing and genericity claim as one hard gate. |
...
- [Bootstrap AppBootstrap Enforcement](docs/project/architecture/enforcement/bootstrap-app-bootstrap-enforcement.md:1)
