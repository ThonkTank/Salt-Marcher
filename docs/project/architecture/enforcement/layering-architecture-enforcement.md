...
| `layering-intentional-cross-layer-public-boundary-set` | Enforced Elsewhere | every type that acts as a public layer-crossing boundary or registration root | shell `shell-api-public-surface-allowlist`; view `view-layer-contribution-count`, `view-layer-binder-count`, `view-contribution-discovery-entrypoint-shape`, and `view-binder-dependency-boundary`; domain `domain-applicationservice-root-presence`, `domain-published-direct-file-placement`, `domain-port-role-shape`, and `domain-port-published-listener-boundary`; data `data-root-service-contribution-only` and `data-service-contribution-discovery-entrypoint-shape` | see neighboring owner docs and their listed entrypoints | The global layer model contains only the documented public cross-layer boundary families: `shell/api/**`, shell runtime composition surfaces, view `*Contribution` roots, view `*Binder` roots, root domain `*ApplicationService` boundaries, root domain `published/**` carriers, and data `*ServiceContribution` registration roots. |
...
| `layering-no-substantive-domain-role-pass-through-wrapper` | Review-Owned | every thin domain orchestration or helper owner under `src/domain/**` | none | none | The target domain role family does not keep ceremony-only pass-through owners whose only purpose is diagram symmetry. This remains a review claim until the old tactical-role relay diagnostics are replaced by target-role-specific blockers. |
