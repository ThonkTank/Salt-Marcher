...
| `layering-no-substantive-domain-role-pass-through-wrapper` | Enforced Elsewhere | every tactical domain `service/`, `policy/`, or `factory/` role under `src/domain/**` | domain-service bundle PMD `DomainServiceNoCeremonialIndirectionRule`; domain-policy bundle PMD `DomainPolicyNoCeremonialIndirectionRule`; domain-factory bundle PMD `DomainFactoryNoCeremonialIndirectionRule` | `./gradlew checkDomainServiceEnforcement`, `./gradlew checkDomainPolicyEnforcement`, and `./gradlew checkDomainFactoryEnforcement` | Substantive domain tactical roles do not survive only as relay or constructor-wrapper ceremony when they are expected to own real domain behavior or construction work. |
