...
| Invariant ID | Status | Applies When | Mechanical Owner | Blocking Entrypoint | What It Proves |
| --- | --- | --- | --- | --- | --- |
| `shell-no-feature-or-bootstrap-dependencies` | Enforced | every active Java source under `shell/**` | ArchUnit `ShellLayerArchitectureTest` (`shellMustNotReachFeatureInteractorsDomainOrData`, `shellMustStayIndependentFromBootstrap`, and `shellApiMustStayIndependentFromHostAndFeatureLayers`) | `./gradlew checkShellLayerEnforcement` and `./gradlew checkArchitecture` | Shell code does not depend on `src/view/**`, `src/domain/**`, `src/data/**`, or `bootstrap/**`. The shell stays outside feature implementation and outside bootstrap internals. |
| `shell-no-public-surface-outside-shell-api` | Enforced | every public shell-facing Java source | shell-layer bundle build-harness `ShellLayerTopologyRules` | `./gradlew checkShellLayerEnforcement` and `./gradlew checkArchitecture` | Public shell contracts do not escape into arbitrary shell packages; shell-facing extension points stay confined to the fixed `shell/api/**` surface. |
