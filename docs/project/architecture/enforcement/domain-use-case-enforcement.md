...
| `domain-usecase-no-root-usecase-chains` | Enforced | every root `application/*UseCase.java` under `src/domain/**` | domain-usecase bundle Error Prone `DomainUseCaseRoleBoundary` | `./gradlew compileJava` and `./gradlew checkDomainEnforcement` | Root use cases do not depend on other root use cases or their nested types; one-family work stays in model-family `usecase/`, and the root `ApplicationService` routes to the correct root operation directly. |
