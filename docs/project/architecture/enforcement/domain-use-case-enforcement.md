...
| Invariant ID | Status | Applies When | Mechanical Owner | Blocking Entrypoint | What It Proves |
| --- | --- | --- | --- | --- | --- |
| `domain-usecase-thin-orchestration-semantics` | Review-Owned | every `application/*UseCase.java` under `src/domain/**` | none | none | A mechanically legal use case still reads as thin orchestration rather than a second tactical model layer or service dump. |
...
| `domain-usecase-no-hidden-carrier-bypass-into-model` | Review-Owned | every use case handoff into named domain modules | none | none | No legal type shape is being used to pass view, data, shell, or published carriers into named modules through paths the current dependency checks cannot see. |
