...
| `domain-usecase-no-same-context-published-dependencies` | Enforced | every `application/*UseCase.java` compilation unit under `src/domain/<context>/application/` | domain-usecase bundle Error Prone `DomainApplicationNoSameContextPublishedDependency` | `./gradlew checkDomainUseCaseEnforcement` | Use cases do not depend on their own same-context `published/**` carriers. Same-context carrier translation stays at the root boundary or in direct internal application-boundary helper files. |
...
| `domain-usecase-helper-role-discipline` | Review-Owned | every non-`*UseCase.java` direct helper under `src/domain/<context>/application/` | none | none | Direct application-boundary helpers stay narrow and role-pure: translation, projection, or runtime adaptation only, without becoming generic mapper dumps, second public boundaries, or hidden business-policy owners. |
