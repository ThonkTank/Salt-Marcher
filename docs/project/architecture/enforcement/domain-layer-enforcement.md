...
| `domain-layer-model-subtree-no-technical-buckets` | Enforced | every Java type below `src/domain/<context>/model/<family>/model/**` | domain-layer bundle build-harness `DomainLayerTopologyRules` | `./gradlew checkDomainEnforcement` | The internal model subtree stays semantic. Nested technical buckets such as `published`, `application`, `usecase`, `helper`, `constants`, `port`, `repository`, and rejected legacy role names are blocked there. |
