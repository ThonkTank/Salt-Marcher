...
| `domain-layer-no-ui-translation-or-source-composition-ownership` | Review-Owned | all domain code across `src/domain/**` | none | none | A mechanically legal domain package still does not take ownership of UI translation, persistence mechanics, data-source records, or source-adapter composition. Runtime service registration stays limited to direct context-root service-composition files. |
