...
domain-layer topology and domain-wide communication boundaries in `src/domain/**`.
...
| `domain-layer-public-concrete-type-shape` | Enforced | every public concrete type under a named domain module | Error Prone `DomainPublicConcreteTypeShape` | `./gradlew compileJava` | Public named-module domain types use the allowed project shapes: record, enum, final class, interface, or sealed abstraction. |
...
| `domain-layer-forbidden-top-level-domain-buckets` | Enforced | every direct child bucket under `src/domain/<context>/` | build-harness `SourceLayoutRules` | `./gradlew checkArchitecture` | Legacy technical or tactical buckets such as `repository/`, `gateway/`, `schema/`, `record/`, `model/`, `adapter/`, or top-level `aggregate/` are forbidden at context root. |
...
| `domain-layer-mapcore-removed` | Enforced | repository-wide | build-harness `DomainFeatureRules` | `./gradlew checkArchitecture` | `src/domain/mapcore/**` is absent. Map and world authored truth stay in real owning contexts such as `dungeon`. |
