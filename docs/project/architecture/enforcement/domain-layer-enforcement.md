...
| `domain-layer-internal-model-no-same-context-application-boundary-dependencies` | Enforced | every internal model type under `src/domain/<context>/model/<family>/model/**` | domain-layer bundle ArchUnit `domainInternalModelMustNotReachSameContextApplicationBoundary` | `./gradlew checkArchitecture` and `./gradlew checkDomainLayerEnforcement` | Internal model code does not reach same-context root `ApplicationService` or root `application/**` boundary types directly. |
