...
| `domain-layer-legacy-role-suffix-rejection` | Enforced | every domain type whose simple name ends with a role-indicating suffix outside the closed target role family | domain-layer bundle build-harness `DomainLayerTopologyRules` and domain-layer bundle Error Prone `DomainSourceTopologyPerimeter` | `./gradlew compileJava`, `./gradlew checkArchitecture`, and `./gradlew checkDomainLayerEnforcement` | The topology is closed to the target role family. Any other role-indicating suffix is illegal. |
...
| `domain-layer-internal-model-no-port-or-repository-dependencies` | Enforced | every dependency from `src/domain/<context>/model/<family>/model/**` into same-context `port/` or `repository/` role buckets | domain-layer bundle ArchUnit `domainInternalModelMustNotDependOnPortsOrRepositories` | `./gradlew checkArchitecture` and `./gradlew checkDomainLayerEnforcement` | Internal model code does not depend directly on same-context port or repository roles. Those collaboration seams belong above the internal state subtree. |
