...
| `styling-no-inline-setstyle-backchannel` | Enforced | every active source file under `bootstrap/`, `shell/`, and `src/` | typed Gradle verification task `checkManualNodeStyling` and Error Prone `ViewManualNodeStyling` | `./gradlew checkStylingEnforcement`, `./gradlew compileJava`, and `./gradlew check` | Active application code does not communicate styling through `setStyle(...)` as an inline backchannel around centralized stylesheet ownership. |
