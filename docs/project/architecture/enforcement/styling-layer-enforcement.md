...
| `styling-no-inline-setstyle-backchannel` | Enforced | every active source file under `bootstrap/`, `shell/`, and `src/` | Error Prone `ViewManualNodeStyling` | `./gradlew compileJava`, `./gradlew checkStylingEnforcement`, and `./gradlew check` | Active application code does not communicate styling through `setStyle(...)` as an inline backchannel around centralized stylesheet ownership. |
