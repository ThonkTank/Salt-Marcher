...
| Invariant ID | Status | Applies When | Mechanical Owner | Blocking Entrypoint | What It Proves |
| --- | --- | --- | --- | --- | --- |
| `domain-helper-constants-only-downward-dependency` | Enforced | every helper under `src/domain/**` | domain-helper bundle Error Prone `DomainHelperRoleBoundary` | `./gradlew compileJava` and `./gradlew checkDomainEnforcement` | Helpers reference only same-context `Model` input types, same-context `Constants`, passive platform types, and their own nested types. This does not prove that the helper is a necessary or semantically thin work step. |
