...
| Invariant ID | Status | Applies When | Mechanical Owner | Blocking Entrypoint | What It Proves |
| --- | --- | --- | --- | --- | --- |
| `domain-helper-constants-only-downward-dependency` | Enforced | every helper under `src/domain/**` | domain-helper bundle Error Prone `DomainHelperRoleBoundary` | `./gradlew checkDomainEnforcement` | Helpers do not reference forbidden domain roles, known outer-layer types, or executable protocol types. This does not prove that the helper is a necessary or semantically thin work step. |
