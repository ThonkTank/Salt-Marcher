...
| `view-layer-primitives-single-technical-view-root` | Enforced | every reusable `slotcontent/primitives/**` unit under `src/view/**` | build-harness `ViewLayerTopologyRules` | `./gradlew checkViewLayerEnforcement` | A reusable primitive unit defines exactly one top-level technical `*View.java` root; extra peer views are forbidden. |
| `view-layer-primitives-no-non-technical-role-files` | Enforced | every reusable `slotcontent/primitives/**` unit under `src/view/**` | build-harness `ViewLayerTopologyRules` | `./gradlew checkViewLayerEnforcement` | A reusable primitive unit does not define `*ContentModel.java`, `*IntentHandler.java`, `*ViewInputEvent.java`, `*PublishedEvent.java`, or `*InspectorEntry.java`; top-level companions there must use explicit technical support suffixes instead of growing extra view-layer roles. |
...
| `view-layer-primitives-technical-boundary-only` | Enforced Elsewhere | every passive `*View.java` under `src/view/slotcontent/primitives/**` | Error Prone `TechnicalPrimitiveViewBoundary` | `./gradlew compileJava` and `./gradlew checkViewEnforcement` | A technical primitive `View` exposes and retains only technical JavaFX/JDK types, same-unit technical support carriers, and same-type nested carriers. It must not drift into `*ViewInputEvent`, `*PublishedEvent`, `*ContentModel`, `*ContributionModel`, `*IntentHandler`, domain, data, or application-service types. |
