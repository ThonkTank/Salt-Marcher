...
| --- | --- | --- | --- | --- | --- |
| `view-layer-active-root-file-role-allowlist` | Enforced | every active root under `src/view/leftbartabs/**`, `src/view/statetabs/**`, and `src/view/dropdowns/**` | build-harness `ViewTopologyPerimeterRules`; build-harness `ViewLayerTopologyRules`; Error Prone `ViewSourceTopologyPerimeter` | `./gradlew checkViewLayerEnforcement` and `./gradlew checkViewEnforcement` | Active roots may contain only the documented top-level role files, only in the documented direct-file active-root layout, and rename or move bypasses block directly as foreign directory, depth, or role-form violations. |
...
| `view-layer-active-root-no-publishedevent-role` | Enforced | every active root under `src/view/leftbartabs/**`, `src/view/statetabs/**`, and `src/view/dropdowns/**` | build-harness `ViewTopologyPerimeterRules`; build-harness `ViewLayerTopologyRules`; Error Prone `ViewSourceTopologyPerimeter` | `./gradlew checkViewLayerEnforcement` and `./gradlew checkViewEnforcement` | Active roots do not define top-level `*PublishedEvent.java` files. |
| `view-layer-slotcontent-file-role-allowlist` | Enforced | every reusable `slotcontent/**` unit under `src/view/**` | build-harness `ViewTopologyPerimeterRules`; build-harness `ViewLayerTopologyRules`; Error Prone `ViewSourceTopologyPerimeter` | `./gradlew checkViewLayerEnforcement` and `./gradlew checkViewEnforcement` | Reusable `slotcontent/**` units may contain only the documented top-level role files, only in the documented direct-file reusable layout, and rename or move bypasses block directly as foreign directory, depth, or role-form violations. |
...
| `view-layer-slotcontent-reusable-unit-shape` | Enforced | every reusable `slotcontent/**` unit under `src/view/**` | build-harness `ViewLayerTopologyRules`; build-harness `ViewTopologyPerimeterRules`; Error Prone `ViewSourceTopologyPerimeter` | `./gradlew checkViewLayerEnforcement` and `./gradlew checkViewEnforcement` | Each reusable unit defines exactly one top-level `*View.java`, exactly one `*ContentModel.java`, and a same-stem `*ViewInputEvent.java` only when that reusable View is interactive, with no foreign top-level roles. |
...
| `view-layer-no-legacy-view-role-buckets` | Enforced | every active or reusable unit under `src/view/**` | build-harness `ViewTopologyPerimeterRules`; Error Prone `ViewSourceTopologyPerimeter`; PMD `SaltMarcherSourcePolicyRule`; jQAssistant `ViewLayerClosedWorldTopology` | `./gradlew checkViewLayerEnforcement` and `./gradlew checkViewEnforcement` | Legacy `*ViewModel`, `*PresentationModel`, `*Projector`, component-local `View/`, `assembly/`, `Controller/`, `interactor/`, old `api/`, and other foreign role or directory forms are blocked as closed-world topology violations instead of falling through as tolerated legacy buckets. |
...
| `view-layer-intenthandler-direct-applicationservice-write-seam` | Enforced Elsewhere | every active-root domain-write path that originates from `*ViewInputEvent` interpretation inside `src/view/**` | Error Prone `ViewIntentHandlerDependencyBoundary`; ArchUnit `intentHandlersMustDependOnDomainOnlyThroughRootApplicationServices`; Error Prone `ViewBinderApplicationSinkWiring` | `./gradlew compileJava`, `./gradlew checkArchitecture`, `./gradlew checkViewIntentHandlerEnforcement`, and `./gradlew checkViewBinderEnforcement` | Domain writes leave the view layer only through direct active-root `IntentHandler -> *ApplicationService` calls; Binder-injected legacy outward-work sinks are blocked. |
