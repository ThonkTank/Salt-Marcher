...
| --- | --- | --- | --- | --- | --- |
| `view-layer-active-root-file-role-allowlist` | Candidate | every active root under `src/view/leftbartabs/**`, `src/view/statetabs/**`, and `src/view/dropdowns/**` | build-harness `ViewTopologyPerimeterRules`; build-harness `ViewLayerTopologyRules` | none yet | Target architecture allows active roots to contain only documented top-level role files: `*Contribution`, `*Binder`, exactly one aggregate `*ContributionModel`, optional `*IntentHandler`, passive `*View`, same-stem `*ContentModel`, and same-stem `*ViewInputEvent` only for interactive Views. Current gates still need to admit active-root `*ContentModel.java` before this row is fully enforced. |
...
| `view-layer-view-contentmodel-same-stem-required` | Candidate | every passive `*View.java` under `src/view/**` | planned build-harness `ViewLayerTopologyRules` | none yet | Every passive `*View` must have exactly one co-located same-stem `*ContentModel.java`, and every `*ContentModel.java` must belong to exactly one same-stem passive `*View.java`. This is the target blocker that replaces active-root `ContentModel` exclusion. |
...
| Invariant ID | Status | Applies When | Mechanical Owner | Blocking Entrypoint | What It Proves |
| --- | --- | --- | --- | --- | --- |
| `view-layer-active-root-no-contentmodel` | Review-Owned | every active root under `src/view/leftbartabs/**`, `src/view/statetabs/**`, and `src/view/dropdowns/**` | none | none | Current gates still reject active-root `*ContentModel.java` files, but this rule is no longer the target architecture. It must be replaced by same-stem View/ContentModel pairing before product migration. |
...
| `view-layer-view-contentmodel-bind-only` | Candidate | every passive `*View` surface inside `src/view/**` that receives prepared render state | planned Error Prone `PassiveViewSurfaceBoundary`; planned Error Prone `PassiveViewInteractionBoundary` | none yet | Prepared render state should enter a passive `View` only through its own same-stem `bind(SameStemContentModel)` route. Direct `ContributionModel` acquaintance, foreign `ContentModel` acquaintance, project-typed sink payloads, constructor-injected callback relays, and imperative render APIs are not part of the target route. |
