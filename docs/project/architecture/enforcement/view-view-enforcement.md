...
| `view-view-type-shape-boundary` | every passive `*View.java` under `src/view/**` | Error Prone `PassiveViewTypeShapeBoundary` | `./gradlew checkViewEnforcement` | A passive `View` is a concrete UI surface, not a project-View subclass, project-interface adapter, static utility bucket, or constructor-injected callback/model relay. |
...
| `view-view-no-local-data-shaping` | every passive `*View.java` under `src/view/**` | Error Prone `PassiveViewDataShapingBoundary` | `./gradlew checkViewEnforcement` | Passive `View` code does not locally rebuild presentation facts through stream/collector/comparator sorting pipelines, JavaFX observable collection synthesis, or similar data-shaping APIs that belong in the owning model or readback path. |
