...
| `view-contentmodel-no-foreign-view-role-or-foreign-unit-communication` | Enforced | every `*ContentModel.java` under `src/view/**` | Error Prone `ViewContentModelDependencyBoundary` | `./gradlew compileJava` and `./gradlew checkViewEnforcement` | A `ContentModel` does not communicate directly with `*Contribution`, `*Binder`, `*IntentHandler`, foreign `*ContentModel`, `*View`, `*ViewInputEvent`, `*PublishedEvent`, `*InspectorEntry`, or foreign view-unit role families. |
