...
| `domain-context-hex-owned-authored-map-truth` | Review-Owned | the `hex` bullet in `## Context Roles` and the matching `src/domain/hex/DOMAIN.md` contract | none | none | The `hex` authored map contract explicitly owns overworld-style map metadata, tile coordinates, terrain overrides, and simple tile-owned markers while keeping runtime travel and compact `Reise` state outside authored persistence. |
...
| `domain-context-hex-owned-local-authored-map-boundary` | Review-Owned | the `hex` bullet in `## Context Relationships` and the matching `src/domain/hex/DOMAIN.md` contract | none | none | The `hex` context documents authored map truth independently of Dungeon authored maps, party travel state, runtime travel, encounter simulation, and compact `Reise` state. |
| `domain-context-dungeon-consumes-party-travel-position` | Review-Owned | the `dungeon` bullet in `## Context Relationships` and the matching `src/domain/dungeon/DOMAIN.md` contract | none | none | The `dungeon` context documents authored truth independently of `party`, `creatures`, and `encounter`; party-owned travel-position facts reach dungeon runtime travel only through dungeon-owned external boundaries over party published state. |
