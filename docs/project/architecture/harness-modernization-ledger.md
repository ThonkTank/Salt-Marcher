...
| --- | --- |
| Branch | `main` |
| Milestone | T4 - CI authority and bespoke-layer deletion (Done; roadmap superseded) |
| Conversion batch | None |
| Status | Closed - superseded |
| Required next proof | None. T4 is Done and this roadmap is `Status: Deprecated`; T5 and T6 are absorbed by the [Verification Greenfield Roadmap](verification-greenfield-roadmap.md) (T6 as milestone M5, T5 as its deliberately unscheduled Deferred Annex). Continue in the [Verification Greenfield Ledger](verification-greenfield-ledger.md). |
...
| T4 CI authority and bespoke-layer deletion | Done | `4946450b3`, `d528d0b13`, `712ac4f87` | `ea6e797d`, `8aa8ed350` | Local structural proof and Phase 1/Phase 2 approved; build-wiring PR CI forced `check --rerun-tasks` green; area-touch PR CI content-addressed cache behavior green; branch-protection readback Qualified; deleted selector files absent on `main`; first scheduled nightly after the `17 2 * * *` cron recorded green: `gh run view 29307758537 --json headSha,event,conclusion,createdAt,updatedAt,jobs` returned `event=schedule`, `conclusion=success`, `headSha=093a7f96e8565458af4cf9c2935f7a0d4d7ee540`, job `nightly-rerun-tasks=success`, `createdAt=2026-07-14T05:08:48Z`, `updatedAt=2026-07-14T05:15:25Z`, 2026-07-14; its job log records `[observable-gradle] Command: ./gradlew check --console=plain --continue --rerun-tasks` and `BUILD SUCCESSFUL in 5m 44s`, `74 actionable tasks: 73 executed, 1 up-to-date`, 2026-07-14 | PR #453 replaced the required CI surface with `check`, adds scheduled `nightly-rerun-tasks`, deletes `harness-map.json`, `select_harnesses.py`, and `behavior-gate`, removes `checkHarnessMapConsistency`, and updates required-check/frozen/governance surfaces. The prior block was external schedule timing only; the required scheduled evidence now exists and is green, recorded without substituting a manual, PR, or push run. |
