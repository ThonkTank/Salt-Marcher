...
| Field | Value |
| --- | --- |
| Branch | `codex/verif-greenfield-m1b-parallel-safety` |
| Milestone | M1b - Parallel Local Execution (Non-Frozen Surfaces Only) |
| Status | Done on branch |
| Required next proof | After this branch merges, complete the final M1b timing comparison: two consecutive parallel full `check --rerun-tasks` runs green with matching verdicts/executed-task set, at least 40% below the M0 serial baseline, and V9 still holding. |
| Last status note | `2026-07-14 M1b-parallel-safety-on-branch` |
