...
| Field | Value |
| --- | --- |
| Branch | `codex/verif-greenfield-m1b-parallel-settings` |
| Milestone | M1b - Parallel Local Execution (Non-Frozen Surfaces Only) |
| Status | In Flight |
| Required next proof | Parallel-safety rehearsal: prove two behavior tasks run concurrently with isolated `XDG_DATA_HOME` and no shared write paths, then record the literal proof before the final M1b timing comparison. |
| Last status note | `2026-07-14 M1b-parallel-settings-on-branch` |
