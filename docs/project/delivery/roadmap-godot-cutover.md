...
| `G2` Knowledge and table foundation | Native shell plus Campaign objects, Catalog, Creatures, Items, Roster/current Party/planning Party, note-first records, explicit deletion, and shared-definition read semantics. | Delete corresponding JavaFX views, Java application/domain implementations, SQLite adapters, CSS, and shell contributions. | In progress: the production Godot shell has one Katalog route with seven retained sections; Creature and Item metadata use bounded Shared-Definition queries through a latest-wins background controller. Five provider sections remain truthfully unavailable. Campaign objects, provider-owned details/actions, Roster/Party, note-first records, deletion semantics, full paging/sort, visible acceptance, and all corresponding Java/JavaFX/SQLite deletion remain. |
