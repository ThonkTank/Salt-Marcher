...
- native Katalog foundation proof covers one production shell route, all seven
  ...
  one active plus one latest-wins pending worker, stale-result suppression,
  retained per-section input, released worker state, and explicit unavailable
  behavior without Catalog-owned replacement truth;
- native World Planner provider proof covers independently identified
  ...
  Catalog create/edit/trash/restore journey. Encounter and Encounter Tables
  remain explicit unavailable sections;
