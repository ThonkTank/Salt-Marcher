...
- native World Planner provider proof covers independently identified
  ...
  removal/relink, and faction/place Encounter Table references without
  endpoint deletion. Its narrative proof additionally
...
- native Encounter Table proof covers Campaign-partition validation, unique
  weighted Creature membership, bounded Catalog summaries, complete details,
  serial create/edit, stable restart readback, provider-backed selection,
  exact Creature fact resolution, XP ceilings, per-table weight context,
  latest-wins cancellation, released worker state, and the visible weighted
  ledger journey;
- native saved Encounter proof covers Campaign-partition validation, ordered
  ...
  phases. Free-form runtime generation, Session
  Planner planning facts, and combat remain unfinished;
