...
| `G2` Knowledge and table foundation | Native shell plus Campaign objects, Catalog, Creatures, Items, Roster/current Party/planning Party, note-first records, explicit deletion, and shared-definition read semantics. | Delete corresponding JavaFX views, Java application/domain implementations, SQLite adapters, CSS, and shell contributions. | In progress: the production Godot shell has one Katalog route with seven retained sections. Creature and Item metadata use bounded Shared-Definition queries; NPCs, factions, and places use a Campaign-owned immutable partition with stable identity, name-only create, name/note edit, recoverable delete/restore, and a serial generation-bound writer. Party now owns a native Campaign Roster/current-Party partition with a top-bar dropdown, stable duplicate-name identities, optional profile facts, explicit membership, XP/rest progression, recoverable deletion/restore, and bounded async read/write lanes. Encounter and Encounter Tables remain truthfully unavailable. Rich provider details/actions, concrete Party travel/Scene integration, Adventuring Day, Session Planner-owned Planning Party, note-first Quest/rumour records, full paging/sort, visible owner acceptance, and corresponding Java/JavaFX/SQLite deletion remain. |
