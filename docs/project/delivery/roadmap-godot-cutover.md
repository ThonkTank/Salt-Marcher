...
| `G4` Live table | Running Scenes, Encounters, initiative/HP/turns, masks, split Party state, independent time, live notes/search/music, and passive display preserve and resume exact runtime truth. | Delete Scene, Encounter, Encounter Table, Travel state-shell, and related Java owners. | In progress: one native Encounter route opens saved plans through current Creature facts, captures the active Party, persists editable/rollable initiative, individual enemy HP, exact round/active turn, results, and one atomic Encounter-plus-Party XP award through restart. Scene-owned context synchronization, mobs/reinforcements, masks, split Party, independent time, live notes/search/music, passive display, visible owner acceptance, and legacy deletion remain. |
...
- native saved Encounter proof covers Campaign-partition validation, ordered
  ...
  overflow legible without horizontal scrolling. Free-form runtime generation,
  Scene synchronization, mobs/reinforcements, and final cockpit composition
  remain unfinished;
