...
| --- | --- | --- | --- |
| `G0` Target and executable foundation | Godot project starts; Campaign desk creates, lists, activates, restarts, rejects stale activation, and recovers a corrupted newest registry generation through immutable JSON files. Target architecture and persistence contract name no SQLite solution. | None yet: this is the shared foundation and not feature parity. | In progress: code, headless contract/UI journeys, injected write/publication failures, production-scene startup, and a 1366 x 768 render are green. A 1,000-Campaign Linux fixture qualifies background list/open and bounded 50-row UI paging; OS-durable publication and Windows/macOS fault qualification remain. |
...
| `G4` Live table | Running Scenes, Encounters, initiative/HP/turns, masks, split Party state, independent time, live notes/search/music, and passive display preserve and resume exact runtime truth. | Delete Scene, Encounter, Encounter Table, Travel state-shell, and related Java owners. | In progress: the native Scene bridge deck follows the canonical primary/split-group model. One primary group always exists, every active Party PC is assigned exactly once, and the same character-move action splits or reunites groups while empty groups retire automatically. The earlier development-only named/create/delete/prepared-copy assumptions were removed. Party membership, Scene composition, and complete Scene-keyed Encounter replacements publish atomically; World Planner NPC/place references, Creature-backed mobs, participant state, focus, exact Encounter deep links, restart truth, and compatible combat-state reconciliation are covered. Catalog Creature additions now also create live Encounter reinforcements while preserving the active turn and creation roster, or increment the focused Scene mob and reconcile its live context. The JavaFX/SQLite Scene owner, its store/composition, resources, and Java Catalog handoffs are deleted. Encounter's manual route remains native and restart-safe. Location-derived persistent monster groups, masks, independent time, live search/music, passive display, visible owner acceptance, and remaining Encounter/Travel legacy deletion remain. |
...
- add a cross-platform publication boundary which durably synchronizes fresh
  file bytes and their parent-directory namespace before acknowledging a commit;
- qualify abrupt power loss around that boundary on real Windows and macOS in
  addition to Linux. Godot 4.6.2's current `fflush()` plus Windows `MoveFileW`
  route is process-crash/readback safe but does not prove `TN-15` durability.
...
- the Campaign desk loads registry state off the main thread and materializes at
  most 50 sorted rows per page. A 1,000-complete-Campaign Linux fixture (2,001
  files / 995,329 bytes), after five warmups and 100 exact measured runs,
  records list p95/max at 55,613/63,191 µs and selected-open p95/max at
  3,615/4,228 µs against one-second/ten-second budgets;
- real OpenGL production renders completed at 1366 x 768 on Intel UHD 620 and
  were visually inspected for clipping, hierarchy, focus, empty-state copy, and
  the 120-Campaign paging footer;
...
- native Katalog foundation proof covers one production shell route, all seven
  retained section identities, bounded stable-id Creature metadata search,
  stable ordering before bounded 57-record paging, retained
  ...
  Items proof additionally covers every required semantic filter/sort,
  complete filter options, invalid-query and missing/incompatible states,
  off-thread exact detail with attribution, damaged-object isolation, full
  equipment-plus-magic-item validation before storage, unchanged prior truth
  after an incomplete fetch, and one atomic whole-kind generation replacement
  preserving Creature definitions. A live public-source run loaded all 599
  referenced details over one reused TLS connection and published 237 Equipment
  plus 362 Magic Items; a repeated complete replacement reused the 599
  content-addressed objects and selected the next checksummed generation.
  Real OpenGL renders at 1150 x 700 and 900 x 650 keep the generic sections
  usable. A current 1366 x 768 full-599-Item render additionally keeps the
  two-line semantic filter ledger, provider-declared result columns, attributed
  Inspector, and complete shared paging footer visible without horizontal
  scrolling;
- native Creature-owner proof covers a fully validated 325-record SRD fixture,
  combined multi-value classification/environment/alignment and CR filtering,
  complete filter options, stable semantic sorting before 50-row paging,
  invalid-query and cancellation states, exact attributed statblocks, unpaged
  complete current-facts snapshots, damaged-object isolation, unchanged prior
  truth after an incomplete corpus, and one atomic whole-kind replacement that
  preserves Items. Twenty warm 325-record queries meet the two-second p95
  budget. A live public Open5e V2 run fetched all four pages, validated all 325
  `srd-2014` statblocks and their license record, and published one complete
  generation. The real 1366 x 768 OpenGL render on Intel UHD 620 keeps the
  two-row Creature filter ledger, four semantic columns, first 50-row page,
  attributed exact Inspector, and shared paging footer visible without
  horizontal scrolling. Owner acceptance and legacy deletion remain;
...
- native saved Encounter proof covers Campaign-partition validation, ordered
  ...
  no-empty primary initialization, derived group labels, the same-action
  split/reunite flow, exact active-Party assignment in atomic Party-plus-Scene-
  plus-Encounter commits, NPC/place/mob assignment, participant state, exact
  context deep links, independent combat, atomic context reconciliation,
  latest-wins read cleanup, and restart. Inspected real OpenGL renders at 1366
  x 768 and 1150 x 700 keep the focus compass, notes/location, group-move
  controls, mob/NPC state, Encounter
  summary, terminal status, and vertical overflow legible without horizontal
  scrolling. The manual builder proof additionally covers separated persisted
  Catalog filters/Encounter tuning, deterministic bounded alternatives,
  restart-safe diagnostics and selection, visible previous/next, history clear,
  and save-current through one ordinary plan commit. Grouped source proof adds
  searchable Catalog table/faction/location selection, restart persistence,
  direct-table and World-source intersection, current weighted membership,
  finite faction stock, source diagnostics, and non-blocking Loot conflict. A
  real 1366 x 768 OpenGL render keeps the expanded tuning docket, brass
  alternative navigator, source proof, warning, diagnostics, and roster
  controls legible in one vertically scrolling stage. Masks, passive display,
  and final cockpit composition remain unfinished;
