...
| `src/view/statetabs/travel/**` | Route gap: `hexTravelStateBehaviorHarness` publishes a harness-created `HexTravelSnapshot` through a mutable feed instead of the Hex/Party production publication route into the Reise state tab. | P1 | Drive the production travel owner and read back the real `HexTravelModel`, or retain the current task as render-only proof and add a production-route travel state-tab harness. |
| `src/view/leftbartabs/hexmap/**` | Route gap: Hex Map UI route claims include manually assembled content models, views, intent handlers, and hand-built input events/direct handler calls outside shell-bound contribution wiring. | P1 | Move the claimed UI route checks through the actual contribution/shell-bound views, or relabel direct-handler pieces as component invariants and add a production-route Hex Map UI harness. |
