...
though the external Looper can operate continuously.
Target state: `tools/looper-system/docs/looper-system.md` remains the selection
...
would split work selection truth; editing external Looper files was rejected
...
code, CI gates, branch protection, Looper state, or migration behavior.
...
## 2026-07-06 cost-only-stop-looper - Make repair the default loop action

Problem: the live looper still turned quota, red-check, risk-protocol, and
...
the installed Looper prompt, and the installed looper policy; do not weaken
required checks or make red/skipped PRs mergeable.
Done when: docs enforcement passes, looper syntax/dry-run pass, independent
...
Problem: the Looper still allowed `no_work` as a normal result,
...
Scope boundary: change only the Dauerbetrieb contract, repo-owned Looper
artifacts, installed Looper prompt/wrapper, and this journal note; do not
change session pause defaults or cost-only-stop rules.
Instruction volume payment: `looper-system.md` replaces the old `no_work` escape
...
Done when: documentation enforcement passes, the Looper syntax and dry-run
