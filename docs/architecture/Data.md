...
> **Lies auch:** [Features.md](Features.md), [Conventions.md](Conventions.md), [EventBus.md](EventBus.md)
> **Wird benoetigt von:** Features, Infrastructure

Rohe Daten und kontextunabhängige Logik für alle Layer: Schemas, Types, Konstanten, Utilities.

**Pfad:** `src/core/` (historischer Pfadname)

---

## Single-Concern-Regel

**Eine Datei im Data Layer behandelt genau EIN Concern:**

| Concern | Beispiel |
|---------|----------|
| Ein Schema | `MapSchema.ts`, `CreatureSchema.ts` |
| Ein Type-Set | `branded-types.ts`, `creature-types.ts` |
| Ein Konstanten-Set | `xp-thresholds.ts`, `encounter-difficulty.ts` |
| Eine Utility-Gruppe | `hex-math.ts`, `time-math.ts` |

**Data Layer enthält:**
- **Schemas** - Zod-Definitionen für Validierung
- **Types** - TypeScript-Typen, Enums, Unions
- **Konstanten** - XP-Tabellen, Encounter-Schwellwerte, D&D-Regeldaten
- **Utils** - Pure Functions mit kontextunabhängiger Business Logic

**Kontextunabhängig bedeutet:** Die Logik ändert sich nicht basierend auf Feature-State oder User-Kontext. D&D 5e XP-Tabellen, Reisegeschwindigkeiten, CR-Berechnungen sind immer gleich.

→ **Event-Struktur:** Siehe [EventBus.md](EventBus.md)
...
├── types/      # Gemeinsame TypeScript-Typen + Konstanten
