...
only their own domain port operations, and the gateway rule proves that
source-adapter public/protected APIs do not leak domain, published, shell, view,
bootstrap, foreign-data, or persistencecore types. They do not prove that method
bodies preserve invariants or that mapper logic is lossless.
