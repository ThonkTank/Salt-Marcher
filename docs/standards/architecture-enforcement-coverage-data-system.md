...
Source of Truth: Compatibility mirror for canonical documentation at `docs/project/architecture/architecture-enforcement-coverage-data-system.md`.

# Data And System Enforcement Coverage Compatibility Mirror
...
This document mirrors the canonical data and system enforcement coverage so
legacy checks that still read `docs/standards/**` see the same rule
inventory, owner mapping, and review-owned boundaries during the migration.
...
| `data-query-read-only-source-shape` | PMD `SaltMarcherDataLayerRoleRule`, Error Prone `DataAdapterRoleContract` and `DataQueryGatewayMutationBoundary` | `./gradlew checkArchitecture` and `./gradlew compileJava` | Query adapters implement read-only lookup/catalog/search domain ports, do not expose public or protected mutation-prefixed methods visible to the source-pattern check, and do not call mutation-prefixed own-feature gateway operations. |
...
| `data-port-adapter-role-contract` | Error Prone `DataAdapterRoleContract` | `./gradlew compileJava` | Public concrete repository/query adapters implement matching own-feature domain ports, use repository ports only from `repository/` and read-only ports only from `query/`, and expose no public or protected adapter methods beyond matching port contracts, including inherited public or protected superclass methods. |
| `data-port-adapter-public-signature-boundary` | Error Prone `DataAdapterPublicSignatureLeak` | `./gradlew compileJava` | Public or protected repository/query adapter signatures, including inherited public or protected superclass methods, do not leak source-local `model/`, `gateway/`, or `persistencecore` types. |
...
| `data-gateway-public-signature-boundary` | Error Prone `DataGatewayReturnTypeBoundary` | `./gradlew compileJava` | Public or protected gateway APIs expose only own-feature source-model records or `java.lang` or `java.util` value and container types. |
...
below. `Enforced` points to a data or system matrix row in this document.
...
| `data-bootstrap-shell-no-source-wiring` | Enforced | Covered by `data-service-registry-root-only`, `system-bootstrap-shell-feature-independence`, and shell or repository feature-independence coverage. |
...
| `data-mapper-translation-only` | Review-Owned | Current gates block concrete source API leakage from mappers, but cannot prove mapping losslessness or absence of domain validation or policy. |
...
| `data-view-shell-private-data-access` | Enforced Elsewhere | Covered by view and shell dependency rules that ban view or shell access to data implementation packages. |
...
- whether repository or query port adapters translate without losing
  invariants
...
- whether source-adapter error handling and transaction boundaries are
  adequate
...
- whether mappers translate rather than owning domain validation,
  normalization, ranking, or policy
- whether data code owns business rules, invariants, ranking policy, or
  presentation policy in shapes that do not create stable dependency,
  signature, or forbidden-token evidence
- whether source-local model records are source shapes rather than renamed
  domain entities
- whether column and field-name constants would reduce real duplication
  instead of introducing broad low-signal literal scans
