...
| Slotcontent roots contain only passive `*View`, optional slot-local `*ViewModel`, reusable `*DisplayModel`, and detail `*InspectorEntry` adapter files; they must not define shell entrypoints or Binders. | `build-harness:check` | build-harness `view-slotcontent-root-shape`, `view-slotcontent-no-shell-entrypoints` |
...
| Detail `*InspectorEntry` adapters may compose `InspectorEntrySpec` with their own slotcontent View/ViewModel/DisplayModel and domain `published` carriers, but must not own service lookup or cross-root wiring. | `compileJava`, `checkViewArchitecture` | Error Prone `FeatureShellApiAllowlist`, `ViewModelFrameworkIndependence`; jQAssistant `saltmarcher:MvvmInspectorEntryDependencies` |
