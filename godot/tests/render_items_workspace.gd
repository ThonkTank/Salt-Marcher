...
var _output_path := "/tmp/saltmarcher-items-workspace.png"
...
func _render() -> void:
	...
	for _attempt in 1800:
		if catalog.section_snapshot("items").get("status", "") in ["ready", "empty", "unavailable", "incompatible", "failed"]:
			break
		await create_timer(0.001).timeout
	var rows: Array = catalog.section_snapshot("items").get("rows", [])
	...
	if error != OK:
		push_error("Items-Workspace-Render konnte nicht gespeichert werden: %d" % error)
		quit(1)
		return
	print("Items-Workspace-Render: %s" % _output_path)
	quit(0)
