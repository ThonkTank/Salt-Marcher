class_name EncounterTableDetailReadController
extends Node

## Latest-wins detail reads for one Campaign-owned Encounter Table.
...
func _start(request: Dictionary) -> Dictionary:
	...
	if start_error != OK:
		...
		_mutex.unlock()
		return {"ok": false, "status": "worker_error", "error": "Encounter-Table-Details konnten nicht geladen werden."}
		...
		if not campaign_state.get("ok", false):
			...
			if not read.get("ok", false):
				...
				if result.get("ok", false):
					var labels := _resolve_entry_labels(
						result["record"],
					...
					if not labels.get("ok", false):
						...
						result["entry_labels"] = labels["entry_labels"]
...
func _resolve_entry_labels(record: Dictionary, generation: int) -> Dictionary:
	var creature_ids: Array = []
	for entry in record.get("entries", []):
		if _cancelled_from_worker():
			return {"ok": false, "status": "cancelled", "error": "Encounter-Table-Detailabfrage wurde ersetzt."}
