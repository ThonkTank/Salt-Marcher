class_name CreatureDetailReadController
extends Node

## Latest-wins Creature detail reads outside the scene-tree thread.
...
func _start(request: Dictionary) -> Dictionary:
	...
	if start_error != OK:
		...
		_mutex.unlock()
		return {"ok": false, "status": "worker_error", "error": "Monster-Details konnten nicht geladen werden."}
...
func _run_query(request: Dictionary) -> void:
	...
	if not before.get("ok", false):
		result = {"ok": false, "status": "storage_error", "error": "Der lokale Monster-Katalog ist nicht lesbar."}
	elif _cancelled_from_worker():
		result = {"ok": false, "status": "cancelled", "error": "Monster-Detailabfrage wurde ersetzt."}
	else:
		var generation := int(before.get("shared_definitions_generation", 0))
		result = CreatureCatalog.new(_data_root).detail(generation, str(request["definition_id"]))
		var confirmed: Dictionary = registry.load_state()
		if (
			result.get("ok", false)
			and (not confirmed.get("ok", false) or int(confirmed.get("shared_definitions_generation", -1)) != generation)
		):
			result = {"ok": false, "status": "stale", "error": "Der Monster-Katalog änderte sich während der Detailabfrage."}
