class_name WorldPlannerNarrativeReadController
extends Node

## Latest-wins read lane for narrative threads attached to one active world object.
...
func query(subject_id: String, include_deleted: bool = false) -> Dictionary:
	_mutex.lock()
	_latest_epoch += 1
	var request := {
		"epoch": _latest_epoch,
...
func _start(request: Dictionary) -> Dictionary:
	...
	if start_error != OK:
		...
		_mutex.unlock()
		return {"ok": false, "status": "worker_error", "error": "Narrative Fäden konnten nicht geladen werden."}
	...
	if not registry_state.get("ok", false):
		...
		if not campaign_state.get("ok", false):
			...
			if not read.get("ok", false):
				...
				result = model.query_narratives_for_subject(
				...
				if result.get("ok", false):
					...
					if (
						...
						result = {"ok": false, "status": "stale", "error": "Die aktive Campaign änderte sich während der narrativen Abfrage."}
					else:
