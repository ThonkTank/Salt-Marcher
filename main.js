...
var CreateSpellModal = class extends import_obsidian16.Modal {
  constructor(app, presetName, onSubmit) {
    super(app);
    this.onSubmit = onSubmit;
  ...
  onOpen() {
    ...
    new import_obsidian16.Setting(contentEl).setName("Grad").setDesc("0 = Zaubertrick").addDropdown((dd) => {
    ...
    new import_obsidian16.Setting(contentEl).setName("Schule").addDropdown((dd) => {
    ...
    new import_obsidian16.Setting(contentEl).setName("Wirkzeit").addText((t) => {
      t.setPlaceholder("1 Aktion").onChange((v) => this.data.casting_time = v.trim());
      t.inputEl.style.width = "12ch";
    });
    new import_obsidian16.Setting(contentEl).setName("Reichweite").addText((t) => {
      t.setPlaceholder("60 Fu\xDF").onChange((v) => this.data.range = v.trim());
      t.inputEl.style.width = "12ch";
    });
    const comps = new import_obsidian16.Setting(contentEl).setName("Komponenten");
    ...
    new import_obsidian16.Setting(contentEl).setName("Angriff").addDropdown((dd) => {
    ...
    const save = new import_obsidian16.Setting(contentEl).setName("Rettungswurf");
    save.addDropdown((dd) => {
    ...
    const dmg = new import_obsidian16.Setting(contentEl).setName("Schaden");
    ...
    new import_obsidian16.Setting(contentEl).addButton((b) => b.setButtonText("Abbrechen").onClick(() => this.close())).addButton((b) => b.setCta().setButtonText("Erstellen").onClick(() => this.submit()));
    this.scope.register([], "Enter", () => this.submit());
  }
  onClose() {
    this.contentEl.empty();
  ...
  makeTokenEditor(parent, title, onAdd, itemsProvider, onRemove) {
    new import_obsidian16.Setting(parent).setName(title).addText((t) => {
      t.setPlaceholder("Begriff eingeben\u2026");
      const input = t.inputEl;
      t.inputEl.style.minWidth = "260px";
      t.inputEl.addEventListener("keydown", (e) => {
...
var LibraryView = class extends import_obsidian17.ItemView {
  constructor() {
    super(...arguments);
    this.mode = "creatures";
    this.cleanups = [];
...
// src/app/main.ts
var SaltMarcherPlugin = class extends import_obsidian18.Plugin {
  async onload() {
