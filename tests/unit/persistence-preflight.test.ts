...
describe('persistence preflight', () => {
  ...
  it('migrates both role Golden Masters and starts ready afterward', () => {
    ...
    expect(migrationRegistryVersion).toBe(19)
    ...
    expect(restarted.databases).toMatchObject([
      { path: campaign, role: 'campaign', schemaVersion: 39 },
      { path: installation, role: 'installation', schemaVersion: 42 }
    ])
  ...
  it('wraps historical settings and increments once when retiring column preferences', () => {
    const path = join(temporaryRoot(), 'installation.sqlite')
    const database = new Database(path)
    database.exec(`
      CREATE TABLE installation_settings (
  ...
  it('preserves a campaign registry revision already initialized at schema 37', () => {
    ...
    expect(database.pragma('user_version', { simple: true })).toBe(42)
    expect(
      database
        .prepare(
          "SELECT value FROM settings WHERE key = 'campaign_registry_revision'"
