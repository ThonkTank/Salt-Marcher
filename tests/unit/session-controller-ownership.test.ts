...
describe('session controller ownership', () => {
  it('keeps mutation races and reference following out of the workspace projection', () => {
    ...
    expect(mutations).toContain('useAsyncCommandCoordinator')
    expect(mutations).not.toContain('latestSnapshotRequest')
    expect(mutations).not.toContain('latestGroupRequest')
