...
describe('CI platform partitions', () => {
  ...
  workflowGate(
    'keeps proc, profile, AppImage, and packaged installation checks Linux-only',
    () => {
  ...
  workflowGate(
    'hands off the exact CI Local artifact without rebuilding it locally',
    () => {
      ...
      expect(handoff).not.toContain(
        "run('packaged', ['pnpm', 'package:local'])"
      )
  ...
  workflowGate(
    'builds Linux once and validates the same receipt in every consumer',
    () => {
  ...
  workflowGate(
    'aggregates every candidate partition for the exact PR head SHA',
    () => {
  ...
  workflowGate(
    'consumes only registry-generated functional and visual E2E matrices',
    () => {
      for (const path of [
        '.tmp/e2e-runs',
        '.tmp/visual-diffs',
  ...
  workflowGate(
    'runs only candidate attestation after the exact SHA reaches main',
    () => {
      const postPromotion = workflow.slice(
        workflow.indexOf('  post-promotion:')
      )
...
function workflowGate(name: string, inspect: () => void): void {
  legitimateLiteralGate({
    name,
    path: '.github/workflows/check.yml',
    owner: 'delivery-workflow',
    rationale:
      'GitHub Actions job topology and command wiring are YAML configuration literals without a TypeScript runtime model.',
    inspect
  })
}
