...
import { useGroupLifecyclePort } from '../../src/renderer/features/session/use-group-lifecycle-port.js'
...
function fixture() {
  ...
  const context = {
    api: {
      scene: {
        executeGroupLifecycle: execute,
        groupLifecycleStatus: status
      }
    },
    campaignWorkspace: {
  ...
  const hook = renderHook(() => useGroupLifecyclePort('original'), {
    wrapper
  })
  return {
