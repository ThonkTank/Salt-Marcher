...
import { useScenePartyCommandPort } from '../../src/renderer/features/scene-desktop/use-scene-party-command-port.js'

const input = {
  commandId: 'command',
  command: {
    kind: 'set-roster' as const,
    input: {
      sceneId: 'scene',
      memberIds: [],
      expectedRevision: 7,
...
function fixture() {
  ...
  const hook = renderHook(() => useScenePartyCommandPort('original'), {
    wrapper
  })
  return {
